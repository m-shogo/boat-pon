...
研究基盤・全券種市場知能の実装順序は[`docs/research-platform-master-plan.md`](docs/research-platform-master-plan.md)、40件の研究と破綻防止契約は[`docs/research-idea-register.json`](docs/research-idea-register.json)を正本とします。Phase N0、Stage F0、Stage F0-Rは完了しました。F0-Rの人間承認gateはv2へhardening済みで、独立sidecarのshadow writer/GCはともにOFF、live collectorやproductionへ接続していません。詳細は[`docs/research-replay-foundation.md`](docs/research-replay-foundation.md)と[`docs/research-replay-rollout.md`](docs/research-replay-rollout.md)を参照してください。N1のschema/migration実装前レビューは[`docs/n1-all-bet-type-payout-review.md`](docs/n1-all-bet-type-payout-review.md)で`CONDITIONAL`完了。N1-A offline foundation・N1-B permanent schema rollout（`n1-settlement.0.1` zero-data適用）に続き、**N1-C Persistent Backfill を実行済み**（Backfill=COMPLETE / Final verification=COMPLETE / N1-C acceptance=CONDITIONAL）: `n1-settlement.0.2`（checkpoint schema、expand-only）を適用し、Option B（explicit evidence pin無し）で全券種settlementを段階backfill（8,168/8,168完了・failed 0）、candidates 8,154,709 / evidence pins 0 / integrity ok / fk 0 / observation-level dup 0 / legacy payout mismatch 0、最終DB≈9.0GB。closure（2026-07-29）で authoritative full verify と +5,153 line 完全 reconciliation（unexplainedDelta=0）を解消。reconciliation が **4 source archive file（2008-2009）の intra-file 重複**（race-level 重複 candidate 4,196、source-data defect・値誤りなし）を surface。**source-duplication closure（append-only）**: `n1-settlement.0.3` で 624 races の重複 observation を `source_duplicate` として canonical original へ mapping。raw immutable（重複は audit 可能）、active canonical 重複 **0/0**、冪等、value conflict 0。恒久 invariant（active canonical race-level uniqueness=0）を組込。live-archive 日次追加も incremental backfill（8,170/8,170）。`data/boat.sqlite`はN1 write 0（read-only fingerprintのみ）、collector/shadow/GC/productionは全OFF。正本は[`reports/n1c-backfill/n1c-final-report.md`](reports/n1c-backfill/n1c-final-report.md)、finding/resolution は[`reports/n1c-backfill/data-quality-finding-duplicate-source-archives.md`](reports/n1c-backfill/data-quality-finding-duplicate-source-archives.md)です。
