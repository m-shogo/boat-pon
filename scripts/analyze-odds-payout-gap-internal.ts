/**
 * Internal odds-payout-gap analyzer. Import only after the canonical settlement preflight passes.
 * Read-only research; no production/BUY/LINE/public/betting behavior changes.
 */
...
if (!existsSync(DB_PATH)) { console.error("[odds-payout-gap] database not found"); process.exit(1); }
...
type GapResult = {
  ...
  hits: number;
  coverageRate: number;
  currentOddsRoi: number;
  payoutRoi: number;
  gap: number;
  avgCurrentOdds: number;
...
DB: [configured research database]
