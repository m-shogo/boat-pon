...
const JSON_REPORT_PATH="reports/same-day-rhythm-market-screen.json";
const MARKDOWN_REPORT_PATH="reports/same-day-rhythm-market-screen.md";
...
try{
  ...
  const report={generatedAt:new Date().toISOString(),safety:{readOnly:true,priorSameDayOnly:true,productionConnected:false},scope:{exactaRaces:evaluations.length},results,matrix,caveats:["同日の現在レースより前だけを状態へ反映","勢い・反発は心理推定ではなく直前成績proxy","race_entries欠測時はflagを立てない","細分化は仮説生成で独立検証ではない"]};mkdirSync("reports",{recursive:true});
  ...
  assertCanonicalDirectory("reports","SAME_DAY_RHYTHM_REPORTS_DIRECTORY_IDENTITY_INVALID");
  verifyExistingOutput(JSON_REPORT_PATH,"SAME_DAY_RHYTHM_PREEXISTING_JSON_IDENTITY_INVALID");
  verifyExistingOutput(MARKDOWN_REPORT_PATH,"SAME_DAY_RHYTHM_PREEXISTING_MARKDOWN_IDENTITY_INVALID");
  atomicPublish(JSON_REPORT_PATH,`${JSON.stringify(report,null,2)}\n`,"SAME_DAY_RHYTHM_JSON_PUBLISH_TEMP_IDENTITY_INVALID","SAME_DAY_RHYTHM_JSON_PUBLISH_DESTINATION_IDENTITY_INVALID");
  atomicPublish(MARKDOWN_REPORT_PATH,`${lines.join("\n")}\n`,"SAME_DAY_RHYTHM_MARKDOWN_PUBLISH_TEMP_IDENTITY_INVALID","SAME_DAY_RHYTHM_MARKDOWN_PUBLISH_DESTINATION_IDENTITY_INVALID");
...
function atomicPublish(path:string,contents:string,tempIdentityErrorCode:string,destinationIdentityErrorCode:string):void{const parentPath=dirname(path);assertCanonicalDirectory(parentPath,"SAME_DAY_RHYTHM_PUBLISH_PARENT_IDENTITY_INVALID");const tempPath=`${path}.tmp-${process.pid}-${randomUUID()}`;let fd:number|null=null;try{fd=openSync(tempPath,"wx",0o600);writeFileSync(fd,contents,"utf8");fsyncSync(fd);closeSync(fd);fd=null;const verifiedTempPath=assertCanonicalSingleLinkRegularFile(tempPath,tempIdentityErrorCode);verifyExistingOutput(path,destinationIdentityErrorCode);assertCanonicalDirectory(parentPath,"SAME_DAY_RHYTHM_PUBLISH_PARENT_HANDOFF_IDENTITY_INVALID");renameSync(verifiedTempPath,path);}finally{if(fd!==null)closeSync(fd);rmSync(tempPath,{force:true});}}
