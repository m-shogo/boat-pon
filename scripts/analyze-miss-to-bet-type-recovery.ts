...
function assertPayoutCompleteness(): void {
  ...
  if (invalid) throw new Error(`MISS_RECOVERY_PAYOUT_COVERAGE_INCOMPLETE ${JSON.stringify(coverage)}`);
}
...
for (const row of rows) {
  ...
  const [r1, r2, r3] = sel(row.result);
  const raceId = row.race_id;

  const trifecta = hasPayout(raceId, "trifecta", `${s1}-${s2}-${s3}`);
  const trio = hasPayout(raceId, "trio", st(s1, s2, s3));
  const exacta = hasPayout(raceId, "exacta", `${s1}-${s2}`);
  const quinella = hasPayout(raceId, "quinella", sp(s1, s2));
  const wide = hasPayout(raceId, "wide", sp(s1, s2));
  ...
  if (r1 === s1 && r2 === s2 && r3 === s3) selAccuracy = "1着完全一致";
