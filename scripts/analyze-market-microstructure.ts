...
function atomicPublish(path:string,contents:string,tempIdentityErrorCode:string,destinationIdentityErrorCode:string):void{const parentPath=dirname(path);assertCanonicalDirectory(parentPath,"MARKET_MICROSTRUCTURE_PUBLISH_PARENT_IDENTITY_INVALID");const tempPath=`${path}.tmp-${process.pid}-${randomUUID()}`;let fd:number|null=null;try{fd=openSync(tempPath,"wx",0o600);writeFileSync(fd,contents,"utf8");fsyncSync(fd);closeSync(fd);fd=null;const verifiedTempPath=assertCanonicalSingleLinkRegularFile(tempPath,tempIdentityErrorCode);verifyExistingOutput(path,destinationIdentityErrorCode);assertCanonicalDirectory(parentPath,"MARKET_MICROSTRUCTURE_PUBLISH_PARENT_HANDOFF_IDENTITY_INVALID");renameSync(verifiedTempPath,path);}finally{if(fd!==null)closeSync(fd);rmSync(tempPath,{force:true});}}
try {
  ...
  assertCanonicalDirectory("reports","MARKET_MICROSTRUCTURE_REPORTS_DIRECTORY_IDENTITY_INVALID");
  verifyExistingOutput(JSON_REPORT_PATH,"MARKET_MICROSTRUCTURE_PREEXISTING_JSON_IDENTITY_INVALID");
  verifyExistingOutput(MARKDOWN_REPORT_PATH,"MARKET_MICROSTRUCTURE_PREEXISTING_MARKDOWN_IDENTITY_INVALID");
  atomicPublish(JSON_REPORT_PATH,`${JSON.stringify(report,null,2)}\n`,"MARKET_MICROSTRUCTURE_JSON_PUBLISH_TEMP_IDENTITY_INVALID","MARKET_MICROSTRUCTURE_JSON_PUBLISH_DESTINATION_IDENTITY_INVALID");
  atomicPublish(MARKDOWN_REPORT_PATH,`${lines.join("\n")}\n`,"MARKET_MICROSTRUCTURE_MARKDOWN_PUBLISH_TEMP_IDENTITY_INVALID","MARKET_MICROSTRUCTURE_MARKDOWN_PUBLISH_DESTINATION_IDENTITY_INVALID");
