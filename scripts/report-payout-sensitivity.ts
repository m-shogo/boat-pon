...
if (!existsSync(DB_PATH)) {
  console.error("[report-payout-sensitivity] DB not found");
  process.exit(1);
}

const verifiedDbPath = assertCanonicalSingleLinkRegularFile(DB_PATH, "payout sensitivity primary database");
...
`).get(...params) as { n: number };
  ...
  if (row.n > 0) {
    throw new Error(
      `PAYOUT_SENSITIVITY_OFFICIAL_SETTLEMENT_INTEGRITY_FAILED: ${row.n} settled race key(s) do not have a supported payout mapping and exactly one positive non-refund official winning settlement`,
    );
  }
}
...
WITH base AS (
  ...
  SELECT
    ...
    SUM(CASE WHEN result IS NOT NULL AND result != '' AND selection = result AND returned = 0 AND payout_odds IS NULL THEN 1 ELSE 0 END) AS missing_payout_hits,
...
function printHelp() {
  console.log(`Usage:
  pnpm exec tsx scripts/report-payout-sensitivity.ts -- --from YYYY-MM-DD --to YYYY-MM-DD [--decision BUY] [--group-by decision|venue|month|venue-month|all] [--json]\n\nRead-only. No external access. ROI uses complete mapped official winning settlements for every non-empty settled denominator row before payout sensitivity metrics are generated.`);
}
