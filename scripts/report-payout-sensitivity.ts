...
`).get(...params) as { n: number };
  ...
  if (row.n > 0) {
    throw new Error(
      `PAYOUT_SENSITIVITY_OFFICIAL_SETTLEMENT_INTEGRITY_FAILED: ${row.n} winning ticket key(s) do not have a supported payout mapping and exactly one positive non-refund official settlement`,
    );
  }
}
