 ...
 * settlement integrity と decision cohort integrity を確認する。
...
if ((row.cohortInvalidRows ?? 0) > 0) {
  console.error("[odds-payout-gap-preflight] FAIL: target research cohort contains non-3連単 or returned/unknown-return historical BUY rows, but downstream payout consumers do not exclude them explicitly");
  process.exit(2);
}
