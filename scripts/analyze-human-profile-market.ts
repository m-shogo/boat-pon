...
const JSON_REPORT_PATH="reports/human-profile-market-screen.json";
const MARKDOWN_REPORT_PATH="reports/human-profile-market-screen.md";
...
try{
  ...
  assertCanonicalDirectory("reports","HUMAN_PROFILE_REPORTS_DIRECTORY_IDENTITY_INVALID");
  verifyExistingOutput(JSON_REPORT_PATH,"HUMAN_PROFILE_PREEXISTING_JSON_IDENTITY_INVALID");
  verifyExistingOutput(MARKDOWN_REPORT_PATH,"HUMAN_PROFILE_PREEXISTING_MARKDOWN_IDENTITY_INVALID");
  atomicPublish(JSON_REPORT_PATH,`${JSON.stringify(report,null,2)}\n`,"HUMAN_PROFILE_JSON_PUBLISH_TEMP_IDENTITY_INVALID","HUMAN_PROFILE_JSON_PUBLISH_DESTINATION_IDENTITY_INVALID");
  atomicPublish(MARKDOWN_REPORT_PATH,`${lines.join("\n")}\n`,"HUMAN_PROFILE_MARKDOWN_PUBLISH_TEMP_IDENTITY_INVALID","HUMAN_PROFILE_MARKDOWN_PUBLISH_DESTINATION_IDENTITY_INVALID");
...
function atomicPublish(path:string,contents:string,tempIdentityErrorCode:string,destinationIdentityErrorCode:string):void{const parentPath=dirname(path);assertCanonicalDirectory(parentPath,"HUMAN_PROFILE_PUBLISH_PARENT_IDENTITY_INVALID");const tempPath=`${path}.tmp-${process.pid}-${randomUUID()}`;let fd:number|null=null;try{fd=openSync(tempPath,"wx",0o600);writeFileSync(fd,contents,"utf8");fsyncSync(fd);closeSync(fd);fd=null;const verifiedTempPath=assertCanonicalSingleLinkRegularFile(tempPath,tempIdentityErrorCode);verifyExistingOutput(path,destinationIdentityErrorCode);assertCanonicalDirectory(parentPath,"HUMAN_PROFILE_PUBLISH_PARENT_HANDOFF_IDENTITY_INVALID");renameSync(verifiedTempPath,path);}finally{if(fd!==null)closeSync(fd);rmSync(tempPath,{force:true});}}
...
function readMetadata(raceId:string,date:string){const path=`data/raw/kyotei24/odds/${date}/${raceId}-odds3t.html`;if(!existsSync(path))return[];const verifiedPath=assertCanonicalSingleLinkRegularFile(path,"HUMAN_PROFILE_METADATA_IDENTITY_INVALID");return parseKyotei24RacerMetadata(readFileSync(verifiedPath,"utf8"));}
