/**
 * audit-roi-mechanism-skip-filter-payout-completeness.ts — research-only/read-only
 *
...
if (!existsSync(DB_PATH)) {
  console.error("[roi-mechanism-skip-filter-payout-preflight] database not found");
  process.exit(1);
}

const verifiedDbPath = assertCanonicalSingleLinkRegularFile(DB_PATH, "roi mechanism skip-filter primary database");
...
console.log(
  `[roi-mechanism-skip-filter-payout-preflight] covered=${result.coveredRaces}/${result.totalRaces} (${result.coverageRate}%) missing=${result.missingRaces} cohortInvalid=${row.cohortInvalidRows ?? 0} invalidNonRefund=${row.invalidNonRefundRows ?? 0} duplicateKeys=${row.duplicateCombinationKeys ?? 0} returnedRows=${row.returnedRows ?? 0}`,
);

if ((row.cohortInvalidRows ?? 0) > 0) {
  console.error("[roi-mechanism-skip-filter-payout-preflight] FAIL: target research cohort contains non-3連単 or returned/unknown-return historical BUY rows, but the downstream ROI analyzer does not exclude them explicitly");
  process.exit(2);
}
...
if ((row.duplicateCombinationKeys ?? 0) > 0) {
  console.error("[roi-mechanism-skip-filter-payout-preflight] FAIL: target cohort contains duplicate race_id × trifecta × combination settlement keys; scalar payout consumers must not choose an arbitrary row");
  process.exit(2);
}
...
if (!result.complete) {
  console.error("[roi-mechanism-skip-filter-payout-preflight] FAIL: complete positive non-refund official trifecta settlement coverage is required; skip-filter payout ROI/verdict interpretation must remain unavailable");
  process.exit(2);
}

console.log("[roi-mechanism-skip-filter-payout-preflight] PASS: official trifecta settlement coverage and line integrity are complete for the skip-filter population");