...
if ((row.invalidSettlementKeyShapes ?? 0) > 0) {
  console.error("[roi-mechanism-skip-filter-payout-preflight] FAIL: target cohort contains malformed official trifecta settlement keys; skip-filter payout ROI/verdict interpretation must remain unavailable");
  process.exit(2);
}
...
console.log("[roi-mechanism-skip-filter-payout-preflight] PASS: official trifecta settlement coverage, settlement shape, and line integrity are complete for the skip-filter population");
