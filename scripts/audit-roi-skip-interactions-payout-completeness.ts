 ...
 * winners are allowed; malformed, duplicate-combination, or refund rows fail
...
console.log(
  `[skip-interactions-preflight] covered=${result.coveredRaces}/${result.totalRaces} (${result.coverageRate}%) missing=${result.missingRaces} invalidNonRefund=${row.invalidNonRefundRows ?? 0} duplicateKeys=${row.duplicateCombinationKeys ?? 0} returnedRows=${row.returnedRows ?? 0}`,
);
...
if ((row.returnedRows ?? 0) > 0) {
  console.error("[skip-interactions-preflight] FAIL: target cohort contains trifecta refund rows, but the downstream analyzer does not model refund semantics explicitly");
  process.exit(2);
}
