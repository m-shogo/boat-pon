...
if ((row.invalidSettlementKeyShapes ?? 0) > 0) {
  console.error("[skip-interactions-preflight] FAIL: target cohort contains malformed official trifecta settlement keys; ROI, residual-effect, and skip-comparison verdicts must remain unavailable");
  process.exit(2);
}
...
console.log("[skip-interactions-preflight] PASS: official trifecta settlement coverage, settlement shape, and line integrity are complete for the exact forward analysis population");
