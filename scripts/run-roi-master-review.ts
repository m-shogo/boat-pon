...
assertCanonicalDirectory("reports", "ROI_MASTER_REVIEW_REPORTS_DIRECTORY_IDENTITY_INVALID");
...
function verifyExistingOutputs(): void {
  if (existsSync(OUT_JSON)) {
    assertCanonicalSingleLinkRegularFile(
      OUT_JSON,
      "ROI_MASTER_REVIEW_JSON_PREPUBLISH_DESTINATION_IDENTITY_INVALID",
    );
  }
  if (existsSync(OUT_MD)) {
    assertCanonicalSingleLinkRegularFile(
      OUT_MD,
      "ROI_MASTER_REVIEW_MD_PREPUBLISH_DESTINATION_IDENTITY_INVALID",
    );
  }
}
...
function atomicPublish(
  ...
  assertCanonicalDirectory(parentPath, "ROI_MASTER_REVIEW_PUBLISH_PARENT_IDENTITY_INVALID");
  ...
  try {
    ...
    assertCanonicalDirectory(parentPath, "ROI_MASTER_REVIEW_PUBLISH_PARENT_HANDOFF_IDENTITY_INVALID");
