...
function atomicPublish(path: string, content: string, tempErrorCode: string, destinationErrorCode: string): void {
  ...
  assertCanonicalDirectory(parentPath, "BET_TYPE_COURSE_PUBLISH_PARENT_IDENTITY_INVALID");
  ...
  try {
    ...
    assertCanonicalDirectory(parentPath, "BET_TYPE_COURSE_PUBLISH_PARENT_HANDOFF_IDENTITY_INVALID");
...
try {
  ...
  // Preflight the complete canonical destination set before the first report is
  // replaced, so an already-invalid sibling destination cannot leave a partial
  // publication. atomicPublish still revalidates each destination at rename.
  mkdirSync("reports", { recursive: true });
  assertCanonicalDirectory("reports", "BET_TYPE_COURSE_REPORTS_DIRECTORY_IDENTITY_INVALID");
  if (existsSync(OUT_MD)) {
    assertCanonicalSingleLinkRegularFile(OUT_MD, "BET_TYPE_COURSE_MD_PREPUBLISH_DESTINATION_IDENTITY_INVALID");
  }
  if (existsSync(OUT_JSON)) {
    assertCanonicalSingleLinkRegularFile(OUT_JSON, "BET_TYPE_COURSE_JSON_PREPUBLISH_DESTINATION_IDENTITY_INVALID");
  }
  atomicPublish(
    OUT_MD,
    markdown,
