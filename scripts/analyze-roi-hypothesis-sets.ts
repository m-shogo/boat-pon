...
const result = spawnSync(process.execPath, ["--import", "tsx", "scripts/analyze-roi-hypothesis-sets-raw.ts"], {
  stdio: "inherit",
  env: process.env,
});
if (result.error) {
  console.error(`[analyze-roi-hypothesis-sets] failed to start raw analyzer: ${result.error.message}`);
  process.exit(1);
}
