...
const payoutRaceByType = new Map<string, Set<string>>();
...
for (const p of payoutRows) {
  ...
  if (p.payout_yen != null && p.payout_yen > 0) payoutIndex.set(key, p.payout_yen);
  if (p.returned === 1) payoutReturnedSet.add(key);
  if ((p.returned === 1 || (p.payout_yen != null && p.payout_yen > 0)) && REQUIRED_BET_TYPES.includes(p.bet_type as typeof REQUIRED_BET_TYPES[number])) {
...
function sortedPair(a: number, b: number): string { return a < b ? `${a}-${b}` : `${b}-${a}`; }
function sortedTrio(a: number, b: number, c: number): string { return [a, b, c].sort((x, y) => x - y).join("-"); }
function payout(raceId: string, betType: string, combination: string): number | null {
  const key = `${raceId}|${betType}|${combination}`;
  if (payoutReturnedSet.has(key)) return null;
...
const STRATEGIES: Strategy[] = [
  { betType: "3連単", name: "3連単: selection そのまま", getBetCombination: ([s1, s2, s3]) => `${s1}-${s2}-${s3}`, dbBetType: "trifecta" },
  { betType: "3連複", name: "3連複: selection 3艇 trio", getBetCombination: ([s1, s2, s3]) => sortedTrio(s1, s2, s3), dbBetType: "trio" },
  { betType: "2連単", name: "2連単: 上位2艇 exacta", getBetCombination: ([s1, s2]) => `${s1}-${s2}`, dbBetType: "exacta" },
  { betType: "2連複", name: "2連複: 上位2艇 quinella", getBetCombination: ([s1, s2]) => sortedPair(s1, s2), dbBetType: "quinella" },
  { betType: "拡連複", name: "拡連複: 上位2艇 wide", getBetCombination: ([s1, s2]) => sortedPair(s1, s2), dbBetType: "wide" },
];
...
function evaluate(strategy: Strategy): StrategyResult {
  ...
  for (const row of rows) {
    ...
    if (payoutReturnedSet.has(key)) { returnedCount++; continue; }
    if (!raceSetForType.has(raceId)) {
      missingCoverageCount++;
      throw new Error(`ALL_BET_TYPE_SCREENING_PAYOUT_COVERAGE_INCOMPLETE ${JSON.stringify({ betType: betTypeDb, raceId })}`);
    }
    validRaces++;
  ...
  function yearROI(year: string): number { return periodROI([...ymStake.keys()].filter((ym) => ym.startsWith(year))); }
  ...
  if (hits < 5 || validRaces < 100) verdict = "coverage不足";
  else if (ROI >= 108 && roiExMaxHit >= 95 && testROI >= 95 && validationROI >= 95 && hits >= 10) verdict = "今すぐ有望";
  else if (ROI >= 103 && roiExMaxHit >= 90 && hits >= 5 && !(trainROI > 110 && validationROI < 90 && testROI < 90)) verdict = "追加検証候補";
  else if (trainROI > 110 && (validationROI < 85 || testROI < 85)) verdict = "危険/過学習";
  else verdict = "本番投入はまだ早い";

  return {
    betType: strategy.betType, strategyName: strategy.name, nRaces: validRaces, totalTickets: validRaces, avgTicketsPerRace: 1,
    totalStake, hits, hitRate, avgPayoutOdds: Math.round(avgPayoutOdds * 100) / 100,
    medianPayoutOdds: Math.round(medianPayoutOdds * 100) / 100, maxHitOdds: Math.round(maxHitOdds * 100) / 100,
    totalReturn, ROI, roiExMaxHit, roiExMax3Hits, roiExMax5Hits, coverageRate, missingCoverageCount,
    missingCoverageNote: "coverage欠損はfail-closed（ROI・ranking・deep-dive候補を生成しない）",
    missingPayoutCount: missingCount, returnedCount, trainROI, validationROI, testROI,
    year2024ROI: yearROI("2024"), year2025ROI: yearROI("2025"), year2026ROI: yearROI("2026"),
    worstMonthROI, goodMonths, badMonths, warnings, verdict,
  };
}

const results: StrategyResult[] = STRATEGIES.map(evaluate);
const ranking = [...results].sort((a, b) => b.ROI - a.ROI).map((r, i) => ({ rank: i + 1, strategyName: r.strategyName, ROI: r.ROI, verdict: r.verdict }));
const deepDiveCandidates = results
  .filter((r) => r.nRaces >= 100 && r.hits >= 5 && r.ROI >= 100 && r.verdict !== "危険/過学習" && r.verdict !== "coverage不足")
  .sort((a, b) => b.ROI - a.ROI).map((r) => r.betType);
const report: ScreeningReport = {
  generatedAt: new Date().toISOString(), dbPath: DB_PATH, totalBuyRaces: rows.length, strategies: results, ranking, deepDiveCandidates,
...
- 5券種すべてのofficial settlement coverageが100%でない場合はfail-closed

## ROI ランキング

| rank | 戦略 | ROI | ExMaxHit ROI | hits | hitRate | 判定 |
|---|---|---|---|---|---|---|
${ranking.map((r) => { const s = results.find((x) => x.strategyName === r.strategyName)!; return `| ${r.rank} | ${r.strategyName} | **${r.ROI}%** | ${s.roiExMaxHit}% | ${s.hits} | ${pct(s.hitRate)} | ${r.verdict} |`; }).join("\n")}

## 戦略別 詳細

${results.map((s) => `### ${s.strategyName}
...
${deepDiveCandidates.length > 0 ? deepDiveCandidates.map((c) => `- **${c}**`).join("\n") : "- 有望候補なし（全券種 ROI < 100 または hit < 5）"}
