...
function runAudit(script: string): number {
  const auditDbPath = assertCanonicalSingleLinkRegularFile(
    verifiedDbPath,
    "SKIPVENUE_SWITCH_HISTORICAL_AUDIT_DB_IDENTITY_INVALID",
  );
...
const audit = runAudit("scripts/audit-skipvenue-historical-payout-completeness.ts");
if (audit !== 0) {
...
const workspace = mkdtempSync(join(tmpdir(), "boat-pon-skipvenue-switch-"));
try {
  ...
  if (!existsSync(workspaceJson)) throw new Error("SKIPVENUE_SWITCH_HISTORICAL_JSON_OUTPUT_MISSING");
  if (!existsSync(workspaceMd)) throw new Error("SKIPVENUE_SWITCH_HISTORICAL_MD_OUTPUT_MISSING");

  const verifiedJsonPath = assertCanonicalSingleLinkRegularFile(
    workspaceJson,
    "SKIPVENUE_SWITCH_HISTORICAL_JSON_OUTPUT_IDENTITY_INVALID",
  );
  const verifiedMdPath = assertCanonicalSingleLinkRegularFile(
    workspaceMd,
    "SKIPVENUE_SWITCH_HISTORICAL_MD_OUTPUT_IDENTITY_INVALID",
  );
  ...
  atomicPublish(
    OUT_JSON,
    json,
    "SKIPVENUE_SWITCH_HISTORICAL_JSON_PUBLISH_TEMP_IDENTITY_INVALID",
    "SKIPVENUE_SWITCH_HISTORICAL_JSON_PUBLISH_DESTINATION_IDENTITY_INVALID",
  );
  atomicPublish(
    OUT_MD,
    markdown,
    "SKIPVENUE_SWITCH_HISTORICAL_MD_PUBLISH_TEMP_IDENTITY_INVALID",
    "SKIPVENUE_SWITCH_HISTORICAL_MD_PUBLISH_DESTINATION_IDENTITY_INVALID",
  );
} finally {
  rmSync(workspace, { recursive: true, force: true });
}

console.log("[skipvenue-switch-historical] PASS: payout preflight passed before isolated analysis publication");
