...
test("同一venue-dayでrace identityが重複したら拒否する",()=>{const rows=venueDay("A",600);rows[11]!.raceId=rows[10]!.raceId;assert.equal(isCompleteVenueDay(rows),false);});
test("target raceが対象日の時刻表に存在しなければattention contextを拒否する",()=>{const target=venueDay("A",600)[0]!;const wrongDay=venueDay("B",602);assert.equal(attentionContext(target,wrongDay),null);});
