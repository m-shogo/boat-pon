 ...
 * 返還行は評価母集団から除外する契約のため、ここへ直接渡された場合も払戻を利益として数えない。
 */
export function realizedPayoutYen(row: DecisionHistoryRow, stakeYen: number): number {
  if (row.returned || row.result !== row.selection || stakeYen <= 0) return 0;
...
export function computeMaxDrawdown(rows: DecisionHistoryRow[]): number {
  const buyRows = rows
    .filter((row) => row.decision === "BUY" && !row.returned)
 ...
 * - ROI/hitRate/sampleSize は結果確定済み・非返還のBUY行のみで計算する
 * - 的中行のROIは公式 payout_yen のみ。欠落時はfail-closedで評価を生成しない
 * - 外れ行のpayout_yen欠落は0円払戻として正常
 * - 返還行はstake/ROI/hitRate/drawdownの母集団から除外する
...
export function buildRuleEvaluationResult(input: BuildRuleEvaluationInput): RuleEvaluationResult {
  ...
  const nonReturnedBuyRows = buyRows.filter((row) => !row.returned);
  const settledBuyRows = nonReturnedBuyRows.filter((row) => row.result != null && row.result !== "");
  ...
  const returned = buyRows.length - nonReturnedBuyRows.length;
  if (returned > 0) warnings.push(`${returned} BUY rows are returned and excluded from roi/hitRate`);
  const unsettled = nonReturnedBuyRows.length - settledBuyRows.length;
