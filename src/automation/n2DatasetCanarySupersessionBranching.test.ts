...
test("dataset settlement preflights reject branching supersession before active filtering can hide the ancestor", () => {
  const root = mkdtempSync(join(tmpdir(), "boat-pon-supersession-branching-"));
  ...
  try {
    ...
    db.prepare(`INSERT INTO settlement_candidates_v2 VALUES
      ('ancestor','2024-06-05:12:R1','trifecta','settled','normal','initial','resolved','official','fixture-v1',
       'obs-a','parse-a','raw-a',?,NULL,NULL,?,?)`)
      .run("a".repeat(64), FIXTURE_TIME, FIXTURE_TIME);
    db.prepare(`INSERT INTO settlement_candidates_v2 VALUES
      ('branch-a','2024-06-05:12:R1','trifecta','settled','normal','source_revision','resolved','official','fixture-v1',
       'obs-b','parse-b','raw-b',?,'ancestor','branch fixture a',?,?)`)
      .run("b".repeat(64), FIXTURE_TIME, FIXTURE_TIME);
    db.prepare(`INSERT INTO settlement_candidates_v2 VALUES
      ('branch-b','2024-06-05:12:R1','trifecta','settled','normal','source_revision','resolved','official','fixture-v1',
       'obs-c','parse-c','raw-c',?,'ancestor','branch fixture b',?,?)`)
      .run("c".repeat(64), FIXTURE_TIME, FIXTURE_TIME);
  } finally {
    db.close();
  }
  ...
  try {
    ...
    assert.deepEqual(canary.blocks, [
      "DATASET_CANARY_SETTLEMENT_SUPERSESSION_BRANCHING_INVALID:ancestor",
    ]);
    ...
    assert.deepEqual(active.blocks, [
      "DATASET_ACTIVE_SETTLEMENT_SUPERSESSION_BRANCHING_INVALID:ancestor",
    ]);
  } finally {
