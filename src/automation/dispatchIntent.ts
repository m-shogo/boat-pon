...
export function validateIntent(input: unknown): { valid: boolean; errors: string[]; intent: DispatchIntent | null } {
  ...
  if (raw.safetyLevel === "L3" && (typeof raw.approvalGrantId !== "string" || raw.approvalGrantId.trim() === "")) errors.push("L3 requires approvalGrantId");
...
export function buildCanonicalRequest(input: CanonicalRequestInput): { request: TaskRequest; errors: string[] } {
  ...
  if (order.indexOf(intent.safetyLevel) < order.indexOf(task.safetyLevel)) errors.push(`intent safety ${intent.safetyLevel} is below catalog safety ${task.safetyLevel}`);
  if (intent.maxDurationSeconds > task.maxDurationSeconds) errors.push(`intent maxDurationSeconds ${intent.maxDurationSeconds} exceeds catalog maxDurationSeconds ${task.maxDurationSeconds}`);
  ...
  const base: Omit<TaskRequest, "requestDigest"> = {
    requestSchemaVersion: REQUEST_SCHEMA_VERSION, requestId, taskId: intent.taskId,
    requestedAction: requestedAction as TaskRequest["requestedAction"], safetyLevel: intent.safetyLevel,
    authoritySha: input.authoritySha, queueDigest: input.queueDigest, createdAt: input.createdAt,
    requestedBy: intent.requestedBy, maxDurationSeconds: intent.maxDurationSeconds,
...
function isValidUniqueIdArray(value: unknown, pattern: RegExp): value is string[] {
  return Array.isArray(value) && value.every((id) => typeof id === "string" && pattern.test(id)) && new Set(value).size === value.length;
}
function isProcessedIntentLedgerValid(ledger: ProcessedIntentLedger): boolean {
  ...
  for (const [index, value] of raw.entries.entries()) {
    if (!isRecord(value) || !hasOnlyKeys(value, PROCESSED_INTENT_ENTRY_KEYS)
      ...
      || typeof value.recordedAt !== "string" || Number.isNaN(Date.parse(value.recordedAt))) return false;
...
export function isRequestReplay(ledger: ProcessedRequestLedger | null, requestId: string): boolean {
  if (!ledger) return true;
  try { assertIdempotencyLedgerValid(ledger); } catch { return true; }
  return ledger.requestIds.includes(requestId);
}
function assertIdempotencyLedgerValid(ledger: ProcessedRequestLedger): void {
  const raw = ledger as unknown as Record<string, unknown>;
  if (!hasOnlyKeys(raw, PROCESSED_REQUEST_LEDGER_KEYS) || !hasValidOptionalMetadata(raw, PROCESSED_REQUEST_SCHEMA_VERSION)) throw new Error("malformed processed request ledger: metadata or unknown field");
  if (!isValidUniqueIdArray(raw.requestIds, REQUEST_ID_RE)) throw new Error("malformed processed request ledger: requestIds");
  if (!isRecord(raw.idempotencyKeys)) throw new Error("malformed processed request ledger: idempotencyKeys");
  for (const [key, value] of Object.entries(raw.idempotencyKeys)) {
    if (!/^[0-9a-f]{64}$/.test(key) || !isRecord(value) || !hasOnlyKeys(value, IDEMPOTENCY_ENTRY_KEYS)) throw new Error("malformed processed request ledger: idempotency entry");
    if (typeof value.requestId !== "string" || !REQUEST_ID_RE.test(value.requestId)
      ...
      || ("evidencePath" in value && (typeof value.evidencePath !== "string" || !AUTOMATION_HISTORY_PATH_RE.test(value.evidencePath)))) throw new Error("malformed processed request ledger: idempotency entry");
    if (!ledger.requestIds.includes(value.requestId)) throw new Error("malformed processed request ledger: idempotency requestId not recorded");
  }
}
export function assertReplayLedgersConsistent(intents: ProcessedIntentLedger | null, requests: ProcessedRequestLedger | null): void {
  ...
  for (const entry of intents.entries as Record<string, unknown>[]) {
    if (!requestIds.has(entry.requestId as string)) throw new Error("cross-ledger mismatch: processed intent requestId not recorded");
  }
}
