...
test("planner fails closed when catalog and executor activate before queue state", () => {
  ...
  assert.ok(plan.blockers.includes("TASK-N2-020:QUEUE_DORMANT_WHILE_CATALOG_AND_EXECUTOR_ACTIVE"));
  assert.deepEqual(plan.activationActions, []);
  assert.equal(plan.invariants.executorRegistrationAndCatalogActivationMustBeAtomic, true);
