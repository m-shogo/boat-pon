...
test("retained gate requires a canonical lowercase sha256 idempotency key", () => {
  for (const idempotencyKey of [undefined, "b".repeat(63), "B".repeat(64), `${validIdempotencyKey}0`]) {
    assert.throws(() => validateRetainedOutputCommit({ changedPaths: [historyPath], expectedRunId: runId, readText: () => history(idempotencyKey) }), /RETAINED_COMMIT_HISTORY_IDEMPOTENCY_KEY_INVALID/u);
  }
...
test("trusted CLI rejects retained history with a malformed idempotency key", () => {
  ...
  try {
    ...
    assert.throws(() => execFileSync(process.execPath, [gateCli, `--run-id=${runId}`], { cwd: root, encoding: "utf8", env: { ...process.env, TRUSTED_GIT_BIN: trustedGitBin, GITHUB_ACTIONS: "false", GITHUB_RUN_ID: "" } }), /RETAINED_COMMIT_HISTORY_IDEMPOTENCY_KEY_INVALID/u);
  } finally {
