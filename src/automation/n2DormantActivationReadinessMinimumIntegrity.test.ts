...
test("activation report rejects digest-valid readiness with a lowered settled-race minimum", () => {
  const readiness = buildN2MarketBaselineReadinessReport({
    acceptedT5RaceKeys: ["2026-08-01:01:R1"],
    settledRaceKeys: ["2026-08-01:01:R1"],
    minimumSettledRaceCount: 1,
  });
  assert.equal(readiness.status, "READY_FOR_N2_020");
  assert.equal(readiness.n2TaskReady, true);
  ...
  assert.ok(report.blockers.includes("READINESS_MINIMUM_SETTLED_RACES_MISMATCH"));
