...
export function auditMacCaptureHostKeepAwakePlist(plist: string): MacCaptureHostKeepAwakeAudit {
  ...
  if (!/<string>-s<\/string>/u.test(plist)) blockers.push("AC_ONLY_SYSTEM_SLEEP_ASSERTION_MISSING");
  if (/<string>-(?:i|d|m|u)<\/string>/u.test(plist)) blockers.push("UNSCOPED_CAFFEINATE_ASSERTION_PRESENT");
  if (!/<key>RunAtLoad<\/key>\s*<true\/>/u.test(plist)) blockers.push("RUN_AT_LOAD_MISSING");
  if (!/<key>KeepAlive<\/key>\s*<true\/>/u.test(plist)) blockers.push("KEEP_ALIVE_MISSING");
  if (!/<key>ProcessType<\/key>\s*<string>Background<\/string>/u.test(plist)) {
    blockers.push("BACKGROUND_PROCESS_TYPE_MISSING");
  }
  if (/pmset|sudo|token|secret|password|LINE_CHANNEL|BUY/iu.test(plist)) {
