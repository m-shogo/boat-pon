...
test("dormant N2 attempt drift blocks activation without consuming another attempt", () => {
  ...
  assert.ok(report.blockers.includes("TASK-N2-020:DORMANT_ATTEMPT_COUNT_NOT_ZERO"));
  assert.deepEqual(report.activationActions, []);
  assert.equal(report.tasks["TASK-N2-020"].attemptCount, 1);
