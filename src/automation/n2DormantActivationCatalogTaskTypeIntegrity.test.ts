...
test("dormant N2 catalog task type drift blocks activation fail closed", () => {
  const catalogTasks = catalog();
  const marketTask = catalogTasks.find((task) => task.taskId === "TASK-N2-020");
  assert.ok(marketTask);
  marketTask.taskType = "baseline-historical";
  ...
  assert.ok(report.blockers.includes("TASK-N2-020:CATALOG_TASK_TYPE_MISMATCH"));
