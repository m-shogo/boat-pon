...
test("same-run terminal history must reference every retained output", () => {
  ...
  const result = validateRetainedOutputCommit({
    ...
    expectedRunId: "123",
    readText: reader({
      [historyPath]: { runId: "123", taskId, outputs: [output], result: "PASS" },
    }),
  });
...
test("terminal history cannot claim retained output from another run", () => {
  ...
  assert.throws(
    () => validateRetainedOutputCommit({
      ...
      expectedRunId: "123",
      readText: reader({
        [historyPath]: { runId: "123", taskId, outputs: [output, retained("999")], result: "PASS" },
      }),
    }),
...
test("history-only change cannot claim retained output from another run", () => {
  ...
  assert.throws(
    () => validateRetainedOutputCommit({
      ...
      expectedRunId: "456",
      readText: reader({
        [historyPath]: { runId: "123", taskId, outputs: [retained("999")], result: "PASS" },
      }),
    }),
...
test("terminal history rejects malformed retained-output references", () => {
  ...
  assert.throws(
    () => validateRetainedOutputCommit({
      ...
      expectedRunId: "123",
      readText: reader({
        [historyPath]: { runId: "123", taskId, outputs: [output, "reports/automation/retained-outputs/123/nested/report.json"], result: "PASS" },
      }),
    }),
...
test("orphan retained output is rejected", () => {
  ...
  assert.throws(
    () => validateRetainedOutputCommit({
      changedPaths: [output, historyPath],
      expectedRunId: "123",
      readText: reader({ [historyPath]: { runId: "123", taskId, outputs: [], result: "PASS" } }),
    }),
...
test("wrong workflow run cannot commit another run retained output", () => {
  ...
  assert.throws(
    () => validateRetainedOutputCommit({
      changedPaths: [output, historyPath],
      expectedRunId: "123",
      readText: reader({ [historyPath]: { runId: "124", taskId, outputs: [output], result: "PASS" } }),
    }),
...
test("duplicate changed paths are normalized", () => {
  ...
  const result = validateRetainedOutputCommit({
    ...
    expectedRunId: "123",
    readText: reader({ [historyPath]: { runId: "123", taskId, outputs: [output], result: "PASS" } }),
  });
  assert.equal(result.retainedPathCount, 1);
...
test("malformed or mismatched history fails closed", () => {
  ...
  assert.throws(
    () => validateRetainedOutputCommit({
      changedPaths: [output, historyPath],
      expectedRunId: "123",
      readText: reader({ [historyPath]: { runId: "999", taskId, outputs: [output], result: "PASS" } }),
    }),
...
test("local mode derives run identity from retained paths", () => {
  ...
  const result = validateRetainedOutputCommit({
    changedPaths: [output, historyPath],
    expectedRunId: "local",
    readText: reader({ [historyPath]: { runId, taskId, outputs: [output], result: "PASS" } }),
  });
