...
test("activation report rejects READY status when n2TaskReady is false", () => {
  const readiness = { ...readyReadiness(), n2TaskReady: false };
  ...
  assert.ok(report.blockers.includes("READINESS_TASK_READY_STATE_INCONSISTENT"));
...
test("activation report rejects source blockers hidden behind READY status", () => {
  const readiness = { ...readyReadiness(), blockers: ["SOURCE_BLOCKED"] };
  const report = buildN2DormantActivationReport({
    readiness,
    catalogTasks: catalog(),
    queueTasks: queue(),
  ...
  assert.equal(report.status, "CONFLICT");
  assert.equal(report.stage, "CONFLICT");
  assert.ok(report.blockers.includes("READINESS_BLOCKERS_WITH_NONBLOCKED_STATUS"));
  assert.deepEqual(report.activationActions, []);
  assert.equal(report.activationPlanningAttemptDelta, 0);
});
