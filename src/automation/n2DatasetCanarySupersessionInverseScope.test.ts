...
test("canary preflight blocks an in-scope successor that supersedes an out-of-scope identity", () => {
  const root = mkdtempSync(join(tmpdir(), "boat-pon-canary-supersession-inverse-scope-"));
  ...
  try {
    ...
    db.prepare(`INSERT INTO settlement_candidates_v2 VALUES
      ('out-of-scope-prior','2025-06-05:12:R1','trifecta','settled','normal','initial','resolved','official','fixture-v1',
       'obs-a','parse-a','raw-a',?,NULL,NULL,?,?)`)
      .run("a".repeat(64), FIXTURE_TIME, FIXTURE_TIME);
    db.prepare(`INSERT INTO settlement_candidates_v2 VALUES
      ('in-scope-newer','2024-06-05:12:R1','trifecta','settled','normal','source_revision','resolved','official','fixture-v1',
       'obs-a','parse-a','raw-a',?,'out-of-scope-prior','fixture cross-race supersession',?,?)`)
  ...
  try {
    ...
    assert.deepEqual(checked.blocks, [
      "DATASET_CANARY_SETTLEMENT_SUPERSESSION_IDENTITY_INVALID:in-scope-newer",
    ]);
  } finally {
