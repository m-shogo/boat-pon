...
test("retained history rejects duplicate output references", () => {
  assert.throws(
    () => validateRetainedOutputCommit({
      ...
      expectedRunId: runId,
      readText: () => JSON.stringify({ runId, taskId, result: "PASS", blocks: [], executed: true, outputDigest, idempotencyKey, authoritySha, outputs: [output, output] }),
    }),
...
test("trusted CLI rejects duplicate output references", () => {
  ...
  try {
    ...
    for (const [relativePath, content] of [
      [output, "{}\n"],
      [history, `${JSON.stringify({ runId, taskId, result: "PASS", blocks: [], executed: true, outputDigest, idempotencyKey, authoritySha, outputs: [output, output] })}\n`],
    ] as const) {
