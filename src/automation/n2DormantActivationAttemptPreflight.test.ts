...
test("N2 activation preflight rejects dormant attempt consumption before readiness", () => {
  const state = queue();
  state.tasks["TASK-N2-020"].attemptCount = 1;

  assert.deepEqual(
    findN2DormantTaskDefinitionDrift(catalog(), state),
    ["TASK-N2-020:DORMANT_ATTEMPT_COUNT_NOT_ZERO"],
  );
});

test("N2 activation preflight rejects dormant retry-budget drift before readiness", () => {
  const state = queue();
  state.tasks["TASK-N2-020"].maxAttempts = 4;
