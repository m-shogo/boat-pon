...
test("history-only commit cannot create terminal evidence for another workflow run", () => {
  ...
  assert.throws(
    () => validateRetainedOutputCommit({
      ...
      expectedRunId: "456",
      readText: reader({
        [historyPath]: { runId: "123", taskId, outputs: [], result: "PASS" },
      }),
    }),
...
test("one workflow run cannot persist two terminal histories", () => {
  ...
  assert.throws(
    () => validateRetainedOutputCommit({
      ...
      expectedRunId: "123",
      readText: reader({
        [first]: { runId: "123", taskId: firstTaskId, outputs: [], result: "PASS" },
        [second]: { runId: "123", taskId: secondTaskId, outputs: [], result: "PASS" },
      }),
    }),
...
test("history task identity must match its append-only filename", () => {
  ...
  assert.throws(
    () => validateRetainedOutputCommit({
      ...
      expectedRunId: "123",
      readText: reader({
        [historyPath]: { runId: "123", taskId: "TASK-N2-012", outputs: [], result: "PASS" },
      }),
    }),
...
test("local mode can inspect multiple histories without pretending to be one workflow run", () => {
  ...
  const result = validateRetainedOutputCommit({
    ...
    expectedRunId: "local",
    readText: reader({
      [first]: { runId: "local-a", taskId: firstTaskId, outputs: [], result: "PASS" },
      [second]: { runId: "local-b", taskId: secondTaskId, outputs: [], result: "PASS" },
    }),
  });
