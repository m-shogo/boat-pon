...
function withInvalidSemanticHash(fn: (path: string) => void): void {
  const root = mkdtempSync(join(tmpdir(), "boat-pon-dataset-semantic-hash-"));
  ...
  db.prepare(`INSERT INTO raw_documents
    ...
    VALUES ('raw-semantic', ?, 100, 'text/plain','shift_jis',NULL,NULL,NULL,'verified',
  ...
  db.prepare(`INSERT INTO parse_runs
    ...
    VALUES ('parse-semantic','raw-semantic','n1-backfill-archive','n1-settlement-parser-v1','modern_seven_display',
  ...
  db.prepare(`INSERT INTO domain_observations
    ...
    VALUES ('obs-semantic',?,'settlement_result','settlement_result','rr-payload-v1',
      'parse-semantic','raw-semantic',NULL,?,?,
  ...
  db.prepare(`INSERT INTO settlement_candidates_v2
    ...
    VALUES ('candidate-semantic-invalid',?,'win','settled','normal',
      'initial','resolved','official_archive','modern_seven_display',
      'obs-semantic','parse-semantic','raw-semantic',?,NULL,NULL,?,?)`)
...
test("dataset settlement preflights reject producer-impossible active candidate semantic hashes", () => {
  withInvalidSemanticHash((path) => {
    const canary = preflightN2DatasetCanarySettlementLineage(path);
    assert.equal(canary.ok, false);
    assert.equal(canary.checkedCandidateCount, 1);
    assert.deepEqual(canary.blocks, ["DATASET_CANARY_SETTLEMENT_LINEAGE_INVALID:candidate-semantic-invalid"]);

    const active = preflightN2AllActiveSettlementLineage(path);
    assert.equal(active.ok, false);
    assert.equal(active.checkedCandidateCount, 1);
