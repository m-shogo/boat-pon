...
test("dataset settlement preflight rejects a self-superseding candidate before active filtering can hide it", () => {
  const root = mkdtempSync(join(tmpdir(), "boat-pon-self-supersession-"));
  ...
  try {
    ...
    db.prepare(`INSERT INTO settlement_candidates_v2 VALUES
      ('self','2024-06-05:12:R1','trifecta','settled','normal','source_revision','resolved','official','fixture-v1',
       'obs-self','parse-self','raw-self',?,'self','invalid self supersession',?,?)`)
      .run("a".repeat(64), FIXTURE_TIME, FIXTURE_TIME);
  } finally {
    db.close();
  }

  const canaryBlock = "DATASET_CANARY_SETTLEMENT_SUPERSESSION_SELF_REFERENCE:self";
  const activeBlock = "DATASET_ACTIVE_SETTLEMENT_SUPERSESSION_SELF_REFERENCE:self";
  try {
