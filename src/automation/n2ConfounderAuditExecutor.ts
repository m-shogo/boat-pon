...
import {
  auditN2ConfoundersAndRejections,
  type N2ConfounderFlag,
  type N2ConfounderRejectionAuditReport,
} from "../research-replay/n2ConfounderRejectionAudit";
import type {
...
export const N2_CONFOUNDER_AUDIT_EXECUTOR_VERSION = "n2-confounder-audit-executor-v1" as const;
...
function confirmationDigestMatches(confirmation: N2EdgeHistoricalConfirmationReport): boolean {
  const { outputDigest, ...body } = confirmation;
...
export function readN2HistoricalTestArtifact(repoRoot: string): {
  ...
  try {
    ...
    return { artifact: null, blockers: ["HISTORICAL_TEST_REPORT_INVALID_JSON"] };
  }
  ...
  if (typeof value.generatedAt !== "string" || !Number.isFinite(Date.parse(value.generatedAt))) {
    blockers.push("HISTORICAL_TEST_GENERATED_AT_INVALID");
  }
  ...
  if (confirmation && !confirmationDigestMatches(confirmation)) blockers.push("HISTORICAL_CONFIRMATION_DIGEST_MISMATCH");
  if (confirmation && !Array.isArray(confirmation.results)) blockers.push("HISTORICAL_CONFIRMATION_RESULTS_INVALID");
  if (confirmation && confirmation.results.length !== confirmation.lockedHypothesisCount) {
    blockers.push(`HISTORICAL_CONFIRMATION_COUNT_MISMATCH:${confirmation.results.length}/${confirmation.lockedHypothesisCount}`);
  }
  if (confirmation && confirmation.confirmedCount + confirmation.rejectedCount + confirmation.insufficientCount !== confirmation.results.length) {
    blockers.push("HISTORICAL_CONFIRMATION_VERDICT_COUNTS_INVALID");
  }
  ...
  if (value.authority?.automaticPromotionAuthorized !== undefined && value.authority.automaticPromotionAuthorized !== false) {
    blockers.push("HISTORICAL_REPORT_PROMOTION_AUTHORITY_INVALID");
  }
  if (value.authority?.productionApplyAuthorized !== undefined && value.authority.productionApplyAuthorized !== false) {
    blockers.push("HISTORICAL_REPORT_PRODUCTION_AUTHORITY_INVALID");
  }
...
/**
 * N2-041 deliberately persists no venue/year concentration table. Therefore a
 * historically confirmed signal cannot be promoted from the N2-042 aggregate
 * audit alone. This blocking flag is structural, not a post-hoc rescue rule.
 */
export function buildN2StructuralConfounderFlags(
  ...
  return results
    ...
    .map((result) => ({
      ...
      detail: "N2-041 aggregate evidence does not retain venue/year concentration evidence; promotion remains blocked until a separately reviewed distribution audit exists.",
    }));
}
...
export function preflightN2RejectionRegistry(
  registryRoot: string,
  planned: readonly Rejection[],
): { ok: boolean; blockers: string[]; alreadyRecordedCount: number } {
  const blockers: string[] = [];
  ...
  try {
    existing = listRecords<Record<string, unknown>>(registryRoot, "rejections");
  } catch (error) {
    return {
      ok: false,
      blockers: [`REJECTION_REGISTRY_READ_FAILED:${error instanceof Error ? error.message.slice(0, 180) : "UNKNOWN"}`],
...
function relativeRegistryOutput(repoRoot: string, absolutePath: string): string {
  ...
  if (!output.startsWith("research/registries/rejections/") || output.includes("..")) {
    throw new Error(`REJECTION_REGISTRY_OUTPUT_OUTSIDE_ALLOWLIST:${output}`);
  }
  return output;
}
...
export function createN2ConfounderAuditExecutor(): Executor {
  return (ctx) => {
    ...
    const spec: ExecutorSpec = {
      ...
      implemented: true,
      inputContract: () => {
        if (ctx.taskStatuses["TASK-N2-041"] !== "PASS") {
          return { ok: false, errors: [`DEPENDENCY_NOT_SATISFIED:TASK-N2-041=${ctx.taskStatuses["TASK-N2-041"] ?? "UNKNOWN"}`] };
        }
        ...
        const flags = buildN2StructuralConfounderFlags(source.confirmation.results);
        audit = auditN2ConfoundersAndRejections({
        ...
        plannedRejections = buildN2GovernanceRejectionRecords({
          sourceArtifactDigest: source.outputDigest,
      ...
      executeReadOnly: () => {
        if (!source || !audit) throw new Error("N2_CONFOUNDER_AUDIT_INPUT_NOT_READY");
        const summary = {
          reportVersion: "n2-confounder-audit-report-v1",
          ...
          confounderCoverage: {
            venueConcentrationEvidenceAvailable: false,
            eraConcentrationEvidenceAvailable: false,
            confirmedHypothesisPromotionBlockedUntilDistributionAudit: true,
      ...
      pitEvidence: (_sdk, artifact) => ({
        status: "PASS",
        validatorId: "n2-confounder-audit-holdout-lineage",
        validatorVersion: N2_CONFOUNDER_AUDIT_EXECUTOR_VERSION,
      ...
      writeArtifacts: (sdk, artifact) => {
        ...
        for (const record of plannedRejections) {
          ...
          if (result.path) {
            try {
              registryOutputs.push(relativeRegistryOutput(ctx.repoRoot, result.path));
            } catch (error) {
        ...
        try {
          atomicWriteJson(join(sdk.repoRoot, REPORT_RELATIVE_PATH), {
            ...
            taskId: ctx.taskId,
            executorVersion: N2_CONFOUNDER_AUDIT_EXECUTOR_VERSION,
