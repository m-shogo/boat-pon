  ...
  return (ctx) => {
    ...
    const spec: ExecutorSpec = {
      ...
      inputContract:()=>{
        ...
        // If discovery yielded no signals, the historical test is a valid no-op and does not read holdout raw data.
      ...
      executeReadOnly:()=>{
        ...
        if(locked.hypotheses.length===0){
          ...
          const summary={reportVersion:"n2-edge-historical-test-report-v1",executorContractVersion:N2_EDGE_HISTORICAL_TEST_EXECUTOR_VERSION,status:"PASS",discoveryArtifactDigest:locked.digest,lockedHypothesisCount:0,noSignalShortCircuit:true,confirmation,rawProgramReadCount:0,databaseWriteCount:0,networkRequestCount:0,authority:{automaticPromotionAuthorized:false,currentBuyConnectionAuthorized:false,lineConnectionAuthorized:false,publicPublishAuthorized:false,automatedBettingAuthorized:false,productionApplyAuthorized:false}};
