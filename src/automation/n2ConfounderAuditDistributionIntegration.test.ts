...
function writeArtifact(root:string,result:N2EdgeHistoricalConfirmationResult,evidence:N2EdgeHoldoutDistributionEvidenceReport){const dir=join(root,"reports/n2");mkdirSync(dir,{recursive:true});const discovery=writeCurrentDiscovery(root,result);const summary={reportVersion:N2_EDGE_HISTORICAL_TEST_REPORT_VERSION,executorContractVersion:N2_EDGE_HISTORICAL_TEST_EXECUTOR_VERSION,status:"PASS",discoveryArtifactDigest:canonicalHash(discovery),cohort:{selectedValidationRaceCount:220,selectedTestRaceCount:220},confirmation:confirmation([result]),distributionEvidence:evidence,databaseWriteCount:0,networkRequestCount:0,authority:{automaticPromotionAuthorized:false,currentBuyConnectionAuthorized:false,lineConnectionAuthorized:false,publicPublishAuthorized:false,automatedBettingAuthorized:false,productionApplyAuthorized:false}};const payload={...summary,generatedAt:"2026-08-08T08:00:00.000Z",outputDigest:canonicalHash(summary)};writeFileSync(join(dir,"n2-edge-historical-test.json"),JSON.stringify(payload,null,2));}
