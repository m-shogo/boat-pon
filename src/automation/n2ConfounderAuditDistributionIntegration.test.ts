...
function writeCurrentDiscovery(root:string){
  const dir=join(root,"reports/n2");mkdirSync(dir,{recursive:true});
  const summary={status:"PASS" as const,scan:{status:"PASS" as const,scanVersion:"n2-edge-hypothesis-scan-v2",signals:[] as unknown[],authority:discoveryAuthority()},revision:"fixture-current"};
...
function writeArtifact(root:string,result:N2EdgeHistoricalConfirmationResult,evidence:N2EdgeHoldoutDistributionEvidenceReport){const dir=join(root,"reports/n2");mkdirSync(dir,{recursive:true});const discovery=writeCurrentDiscovery(root);const summary={status:"PASS",discoveryArtifactDigest:canonicalHash(discovery),confirmation:confirmation([result]),distributionEvidence:evidence,authority:{automaticPromotionAuthorized:false,currentBuyConnectionAuthorized:false,lineConnectionAuthorized:false,publicPublishAuthorized:false,automatedBettingAuthorized:false,productionApplyAuthorized:false}};const payload={...summary,generatedAt:"2026-08-08T08:00:00.000Z",outputDigest:canonicalHash(summary)};writeFileSync(join(dir,"n2-edge-historical-test.json"),JSON.stringify(payload,null,2));}
