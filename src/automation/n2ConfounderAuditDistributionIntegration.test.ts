...
function writeArtifact(root:string,result:N2EdgeHistoricalConfirmationResult,evidence:N2EdgeHoldoutDistributionEvidenceReport){const dir=join(root,"reports/n2");mkdirSync(dir,{recursive:true});const payload={status:"PASS",generatedAt:"2026-08-08T08:00:00.000Z",outputDigest:canonicalHash({source:"n2-041-distribution-fixture",id:result.hypothesisId}),confirmation:confirmation([result]),distributionEvidence:evidence,authority:{automaticPromotionAuthorized:false,productionApplyAuthorized:false}};writeFileSync(join(dir,"n2-edge-historical-test.json"),JSON.stringify(payload,null,2));}
