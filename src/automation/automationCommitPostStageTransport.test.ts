...
test("automation commit rechecks repo-local transport config after task-controlled staging", () => {
  const definition = script.indexOf("assert_trusted_transport_config() {");
  const fetchGuard = script.indexOf("assert_trusted_transport_config", definition + 1);
  ...
  assert.ok(definition < fetchGuard);
