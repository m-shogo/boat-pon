...
const TEXT_EXTENSIONS = new Set([".css", ".html", ".js", ".json", ".txt", ".webmanifest"]);
...
export async function assemblePublicDashboardDeploy(options: {
  ...
  if (options.snapshotDir) {
    ...
    for (const name of ["latest.json", "last-known-good.json"] as const) {
      ...
      if (await pathExists(source)) await cp(source, join(target, name), { force: true });
    }
  }
...
export async function verifyPublicDashboardDeploy(directory: string): Promise<PublicDeployVerification> {
  ...
  for (const path of files) {
    ...
    if (TEXT_EXTENSIONS.has(extname(path)) || basename(path).startsWith("_")) {
...
async function verifyOptionalSnapshots(root: string, files: Set<string>, errors: string[]): Promise<void> {
  ...
  for (const path of OPTIONAL_PUBLIC_DATA_FILES) {
    ...
    try {
      ...
      snapshots.push({ path, dataAsOf: Date.parse(verified.snapshot.dataAsOf) });
    } catch (error) {
...
function isAllowedPublicPath(path: string): boolean {
  if (!path.includes("/")) return ALLOWED_ROOT_FILES.has(path as typeof REQUIRED_ROOT_FILES[number]);
