...
test("BUY outcome pattern report reconciles paper-live to official race_results and keeps exact segment keys private", async () => {
  ...
  try {
    const { stdout } = await execFileAsync("npx", ["tsx", "scripts/analyze-buy-outcome-patterns.ts", "--run-kind", "paper-live", "--min-settled", "30", "--min-roi-delta", "0.15", "--output-public", output, "--retain-private-dir", privateDir], { env: { ...process.env, BOAT_PON_DB_PATH: dbPath }, maxBuffer: 1024 * 1024 });
    ...
    const publicRecord = JSON.parse(await readFile(output, "utf8")) as any;
    ...
    assert.deepEqual(publicRecord.support, {
      ...
      contrastBlocker: null,
      supportedContrastCount: 2,
      supportedDimensionCount: 1,
    });
    assert.equal(publicRecord.noSignalReason, null);
    assert.deepEqual(new Set(publicRecord.signals.map((signal: any) => signal.direction)), new Set(["SUCCESS_EDGE", "FAILURE_REGIME"]));
    assert.ok(publicRecord.signals.every((signal: any) => signal.dimension === "venue"));
    assert.ok(publicRecord.signals.every((signal: any) => Math.abs(signal.roiDelta) === 2));
    assert.ok(publicRecord.signals.every((signal: any) => signal.productionChangeAllowed === false));
...
function createTables(db: DatabaseSync) {
  db.exec(`
    CREATE TABLE decision_history (
      id INTEGER PRIMARY KEY AUTOINCREMENT, race_id TEXT NOT NULL, date TEXT NOT NULL, venue TEXT NOT NULL, race_no INTEGER NOT NULL,
      bet_type TEXT NOT NULL, decision TEXT NOT NULL, selection TEXT NOT NULL, result TEXT, payout_yen INTEGER, returned INTEGER NOT NULL DEFAULT 0,
      model_version TEXT, estimated_hit_rate REAL, ev REAL, current_odds REAL, sample_size INTEGER, run_kind TEXT NOT NULL, created_at TEXT NOT NULL
    );
    CREATE TABLE race_results (race_id TEXT PRIMARY KEY, trifecta TEXT, payout_yen INTEGER, returned INTEGER NOT NULL DEFAULT 0);
  `);
}
function decisionInsert(db: DatabaseSync) { return db.prepare(`INSERT INTO decision_history (race_id,date,venue,race_no,bet_type,decision,selection,result,payout_yen,returned,model_version,estimated_hit_rate,ev,current_odds,sample_size,run_kind,created_at) VALUES (?,?,?,?,?,'BUY',?,?,?,?,?,?,?,?,?,?,?)`); }
function resultInsert(db: DatabaseSync) { return db.prepare("INSERT INTO race_results (race_id,trifecta,payout_yen,returned) VALUES (?,?,?,?)"); }
