...
async function makeFixtureRoot() {
  ...
  const latest = structuredClone(fixture) as PublicDashboardSnapshot; latest.dataAsOf = "2026-08-05T09:00:00.000Z"; latest.generatedAt = "2026-08-05T09:01:00.000Z";
  const fallback = structuredClone(latest); fallback.dataAsOf = "2026-08-05T08:00:00.000Z"; fallback.generatedAt = "2026-08-05T08:01:00.000Z";
  const owner = buildOwnerDashboardSnapshot({ generatedAt: "2026-08-05T09:01:00.000Z", canonicalBranch: "main", mainSha: "70050f70d1a18df7329cc59aa0276797442fa5a3", ciStatus: "PASS", openPrCount: 1, gitCleanliness: "CLEAN", gitUpdatedAt: "2026-08-05T09:00:00.000Z", taskCatalog: { tasks: [{ taskId: "TASK-N2-011", title: "PIT再検証" }] }, queueState: { tasks: { "TASK-N2-011": { status: "PASS", attemptCount: 3, maxAttempts: 3 } } }, currentRun: { updatedAt: "2026-08-05T08:59:00.000Z", lastResult: "PASS", lastTaskId: "TASK-N2-011", blocks: [] } });
...
test("snapshot rollback is rejected and preview workflow remains non-deploying", async()=>{const f=await makeFixtureRoot();try{const latestPath=join(f.snapshotDir,"latest.json"),fallbackPath=join(f.snapshotDir,"last-known-good.json");const latest=JSON.parse(await readFile(latestPath,"utf8")),fallback=JSON.parse(await readFile(fallbackPath,"utf8"));await Promise.all([writeFile(latestPath,JSON.stringify(fallback),"utf8"),writeFile(fallbackPath,JSON.stringify(latest),"utf8")]);await assert.rejects(assemblePublicDashboardDeploy({distDir:f.dist,staticDir:f.staticDir,outputDir:f.output,snapshotDir:f.snapshotDir}),/latest\.json is older/);const workflow=await readFile(".github/workflows/public-dashboard-preview.yml","utf8");assert.doesNotMatch(workflow,/wrangler\s+deploy|pages\s+deploy|cloudflare/i);}finally{await rm(f.root,{recursive:true,force:true});}});
