...
test("standalone public dashboard has an independent Vite entry", async () => {
  const [html, entry, vite] = await Promise.all([readFile(FILES.html, "utf8"), readFile(FILES.entry, "utf8"), readFile(FILES.vite, "utf8")]);
...
test("public shell is read-only and does not import the operational API", async () => {
  const [entry, app] = await Promise.all([readFile(FILES.entry, "utf8"), readFile(FILES.app, "utf8")]);
  const source = `${entry}\n${app}`;
...
test("public shell includes owner overview and research safety landmarks", async () => {
  const app = await readFile(FILES.app, "utf8");
  for (const landmark of ["owner", "research", "methodology", "glossary", "safety"]) assert.match(app, new RegExp(`id=\\"${landmark}\\"`));
  assert.match(app, /OwnerDashboardSummary/);
  assert.match(app, /ResearchCommandCenter/);
  assert.match(app, /GLOSSARY_TERMS/);
  assert.match(app, /canonical authority/);
});
