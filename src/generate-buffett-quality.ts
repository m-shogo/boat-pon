...
function readLatestScores(): LatestScore[] {
  ...
  try {
    ...
    const files = fs.readdirSync("reports").filter((file: string) => /^scores_\d{4}-\d{2}-\d{2}\.json$/.test(file)).sort();
    const latest = files.at(-1);
    if (!latest) return [];
    const parsed = JSON.parse(readFileSync(`reports/${latest}`, "utf-8"));
...
function awaitImportFs() {
  // 静的importを増やさず、既存CLIと同じNode標準だけで完結させる
  // eslintがない環境でも動く単純なrequire代替を避けるため、ここではfs相当を返す。
  return { readdirSync: (path: string) => (existsSync(path) ? (globalThis as any).process.getBuiltinModule?.("fs")?.readdirSync(path) ?? [] : []) };
}
...
function classifyQuality(company: Company, score?: LatestScore): BuffettQualitySnapshot {
  ...
  if (/ネットワーク|プラットフォーム|データセンター|インフラ/.test(text)) {
    moatEvidence.push("ネットワーク/インフラ型の参入障壁候補");
  }
  if (/景気敏感|市況|素材|メモリ|半導体|防衛|重工/.test(text)) {
    moatEvidence.push("景気循環・政策需要の影響が大きく、品質判定は保留");
  }
...
function main() {
  ...
  const output = { generatedAt: todayJst(), snapshots };
  mkdirSync("data", { recursive: true });
  writeFileSync("data/buffett_quality_latest.json", JSON.stringify(output, null, 2), "utf-8");
