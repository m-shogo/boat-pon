...
test("review summary counts and lists only nonblank settled BUY misses", () => {
  const settledMiss = /decision = 'BUY' AND returned = 0 AND result IS NOT NULL AND TRIM\(result\) != '' AND selection != result/g;
