...
test("racer relationship reports preflight both destinations before first replacement", () => {
  ...
  const reportsIdentity = publication.indexOf("RACER_RELATIONSHIP_REPORTS_DIRECTORY_IDENTITY_INVALID");
  const jsonPreflight = publication.indexOf('verifyExistingOutput(JSON_REPORT_PATH, "RACER_RELATIONSHIP_PREEXISTING_JSON_IDENTITY_INVALID")');
  const markdownPreflight = publication.indexOf('verifyExistingOutput(MARKDOWN_REPORT_PATH, "RACER_RELATIONSHIP_PREEXISTING_MARKDOWN_IDENTITY_INVALID")');
...
test("racer relationship publication verifies destination and parent handoff before atomic rename", () => {
  ...
  const parentIdentity = helper.indexOf("RACER_RELATIONSHIP_PUBLISH_PARENT_IDENTITY_INVALID");
  ...
  const parentHandoff = helper.indexOf("RACER_RELATIONSHIP_PUBLISH_PARENT_HANDOFF_IDENTITY_INVALID", destinationIdentity);
