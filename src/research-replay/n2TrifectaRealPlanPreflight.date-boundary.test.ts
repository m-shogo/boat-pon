...
function withTempDb(run: (path: string) => void): void {
  const dir = mkdtempSync(join(tmpdir(), "boat-pon-real-plan-date-boundary-"));
...
test("real plan preflight rejects impossible program dates before the three-date bound", () => {
  withTempDb((path) => {
    ...
    try {
      ...
      insert.run("20260931-05-01", "2026-09-31", "05", 1, "12:00");
    ...
    assert.ok(report.blockers.includes("OFFICIAL_PROGRAM_DATE_INVALID:2026-09-31"));
