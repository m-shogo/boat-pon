...
const internalSource = readFileSync("scripts/report-paper-forward-candidates-internal.ts", "utf-8");
...
test("boundary exists because legacy internal output still embeds configured DB path", () => {
  assert.match(internalSource, /DB: \$\{DB_PATH\}/);
});
