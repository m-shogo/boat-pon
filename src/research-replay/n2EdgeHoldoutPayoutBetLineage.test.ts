...
function withDb(fn: (path: string, db: DatabaseSync) => void): void {
  const root = mkdtempSync(join(tmpdir(), "boat-pon-edge-holdout-payout-lineage-"));
...
test("edge holdout source fails closed when a cross-race successor would hide holdout history", () => {
  withDb((path, db) => {
    const raceKey = "2024-08-01:05:R1";
    const otherRaceKey = "2024-08-02:05:R1";
    seedCandidate(db, raceKey);
    db.prepare(`INSERT INTO settlement_candidates_v2
...
test("edge holdout source fails closed when an in-range candidate points to a missing predecessor", () => {
  withDb((path, db) => {
    const raceKey = "2024-08-01:05:R1";
    seedCandidate(db, raceKey);
