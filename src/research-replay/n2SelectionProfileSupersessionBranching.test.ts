...
test("selection profile rejects branching supersession authority before active candidates are profiled", () => {
  ...
  try {
    ...
    insert.run("ancestor", raceKey, "trifecta", "settled", "normal", "initial", "resolved", "obs-a", "parse-a", "raw-a", null, null, "a".repeat(64));
    insert.run("successor-a", raceKey, "trifecta", "settled", "normal", "source_revision", "resolved", "obs-a", "parse-a", "raw-a", "ancestor", "correction-a", "b".repeat(64));
    insert.run("successor-b", raceKey, "trifecta", "settled", "normal", "official_correction", "resolved", "obs-a", "parse-a", "raw-a", "ancestor", "correction-b", "c".repeat(64));

    assert.throws(
      () => readN2SelectionProfileSource(db, "2026-08"),
      /N2_SELECTION_PROFILE_SUPERSESSION_BRANCHING_INVALID:ancestor/u,
    );
  } finally {
    db.close();
  }
