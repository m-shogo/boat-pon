...
test("ROI hypothesis entrypoint revalidates DB identity after settlement preflight before guarded raw import", () => {
  const close = entrypoint.indexOf("db.close()");
  const handoff = entrypoint.indexOf("ROI_HYPOTHESIS_DB_HANDOFF_IDENTITY_INVALID");
  const rawImport = entrypoint.indexOf('await import("./analyze-roi-hypothesis-sets-raw")');
...
test("ROI hypothesis guarded raw module verifies canonical DB identity before internal analysis", () => {
  const identity = raw.indexOf("ROI_HYPOTHESIS_DB_IDENTITY_INVALID");
  const internal = raw.indexOf('await import("./analyze-roi-hypothesis-sets-internal")');
  ...
  assert.match(raw, /ROI_HYPOTHESIS_DB_MISSING/);
