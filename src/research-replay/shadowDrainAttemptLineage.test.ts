...
test("shadow drain rejects forged terminal attempt history before queue selection", () => {
  const root = mkdtempSync(join(tmpdir(), "shadow-drain-attempt-lineage-"));
  ...
  try {
    ...
    let sequence = 0;
    const repository = new ResearchReplayRepository(db, rawStore, () => `attempt-${++sequence}`, () => now);
    ...
    controller.recordConfig({
      ...
      diskLowWaterBytes: 0,
    }, "fixture-only shadow attempt validation");
    ...
    db.prepare(`
      ...
      VALUES ('forged-attempt-2', 'message-1', 2, 'succeeded', NULL, ?, ?, NULL, ?)
    `).run(
      "2026-08-02T03:10:00.000Z",
      "2026-08-02T03:10:01.000Z",
      "2026-08-02T03:10:01.000Z",
    );
    ...
    const count = db.prepare("SELECT COUNT(*) count FROM shadow_delivery_attempts").get() as { count: number };
