...
function openPrimary(path: string): DatabaseSync {
  const lexicalPath = verifiedDatabasePath(path, "PRIMARY_HOLDOUT_DB_IDENTITY_INVALID");
...
function openSidecar(path: string): DatabaseSync {
  const lexicalPath = verifiedDatabasePath(path, "SIDECAR_HOLDOUT_DB_IDENTITY_INVALID");
