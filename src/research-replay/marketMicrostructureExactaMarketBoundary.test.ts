...
test("market microstructure preflights scalar exacta settlement integrity before rows are aggregated", () => {
  const source = readFileSync("scripts/analyze-market-microstructure.ts", "utf8");
  const preflight = source.indexOf("assertSettlementCoverage(coverage)");
  const analysis = source.indexOf("const rows=db.prepare");

  assert.ok(preflight >= 0 && analysis > preflight, "settlement integrity must pass before market-shape aggregation");
  assert.match(source, /CASE WHEN COUNT\(\*\)=1/);
  assert.match(source, /MARKET_MICROSTRUCTURE_EXACTA_SETTLEMENT_INTEGRITY_INVALID/);
...
test("market microstructure preflights both report destinations before first replacement", () => {
  const source = readFileSync("scripts/analyze-market-microstructure.ts", "utf8");
  ...
  const reportsIdentity = publication.indexOf("MARKET_MICROSTRUCTURE_REPORTS_DIRECTORY_IDENTITY_INVALID");
  const jsonPreflight = publication.indexOf('verifyExistingOutput(JSON_REPORT_PATH,"MARKET_MICROSTRUCTURE_PREEXISTING_JSON_IDENTITY_INVALID")');
  const markdownPreflight = publication.indexOf('verifyExistingOutput(MARKDOWN_REPORT_PATH,"MARKET_MICROSTRUCTURE_PREEXISTING_MARKDOWN_IDENTITY_INVALID")');
...
test("market microstructure publication verifies destination and parent handoff before atomic rename", () => {
  const source = readFileSync("scripts/analyze-market-microstructure.ts", "utf8");
  const helperStart = source.indexOf("function atomicPublish(");
  const tryStart = source.indexOf("try {", helperStart);
  assert.notEqual(helperStart, -1);
  assert.notEqual(tryStart, -1);
  const helper = source.slice(helperStart, tryStart);
  const parentIdentity = helper.indexOf("MARKET_MICROSTRUCTURE_PUBLISH_PARENT_IDENTITY_INVALID");
  ...
  const parentHandoff = helper.indexOf("MARKET_MICROSTRUCTURE_PUBLISH_PARENT_HANDOFF_IDENTITY_INVALID", destinationIdentity);
  ...
  assert.match(source, /MARKET_MICROSTRUCTURE_JSON_PUBLISH_TEMP_IDENTITY_INVALID/u);
  assert.match(source, /MARKET_MICROSTRUCTURE_MARKDOWN_PUBLISH_TEMP_IDENTITY_INVALID/u);
  assert.doesNotMatch(source, /writeFileSync\("reports\/market-microstructure-screen\.(?:json|md)"/u);
});
