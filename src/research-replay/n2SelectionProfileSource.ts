...
function requireSelectionSemantics(
  ...
  if (selectionRaw === null && selectionNormalized === null && selectionCanonical === null) return;
  if (selectionRaw === null || selectionNormalized === null || selectionCanonical === null) {
  ...
  if (!parsed.valid
    ...
    || parsed.canonical !== selectionCanonical) {
    throw new Error(`N2_SELECTION_PROFILE_${errorCode}_SELECTION_INVALID:${candidateId}`);
  }
}
