...
function withTempDb(run: (path: string) => void): void {
  const dir = mkdtempSync(join(tmpdir(), "boat-pon-real-plan-identity-boundary-"));
...
test("real plan preflight rejects invalid race identity before the three-date bound", () => {
  withTempDb((path) => {
    ...
    try {
      ...
      insert.run("20261001-05-99", "2026-10-01", "05", 1, "12:00");
    ...
    assert.ok(report.blockers.includes(
      "OFFICIAL_PROGRAM_RACE_IDENTITY_INVALID:2026-10-01:05:20261001-05-99",
    ));
...
test("real plan preflight rejects duplicate race numbers before the three-date bound", () => {
  withTempDb((path) => {
    const db = new DatabaseSync(path);
    try {
      db.exec(`
      ...
      `);
      insert.run("20260930-05-01", "2026-09-30", "05", 1, "12:00");
      insert.run("20261001-05-01", "2026-10-01", "05", 1, "12:00");
      insert.run("20261001-05-01-copy", "2026-10-01", "05", 1, "12:05");
      insert.run("20261002-05-01", "2026-10-02", "05", 1, "12:00");
      insert.run("20261003-05-01", "2026-10-03", "05", 1, "12:00");
    } finally {
      db.close();
