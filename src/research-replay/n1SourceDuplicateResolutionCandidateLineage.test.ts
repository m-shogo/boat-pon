...
function semanticHash(): string {
  return canonicalHash({
    ...
    resultKind: "normal",
    payouts: [["1-2-3", 4200, null, "payout"]],
    refunds: [],
  });
}
...
function fixture(): DatabaseSync {
  ...
  insertCandidate.run("candidate-canonical", RACE_KEY, "trifecta", "settled", "normal", "initial", "obs-canonical", PARSE_ID, RAW_ID, hash, null, null);
  insertCandidate.run("candidate-duplicate", RACE_KEY, "trifecta", "settled", "normal", "initial", "obs-duplicate", "parse-2", "raw-2", hash, null, null);
...
test("source duplicate evidence rejects candidate parse/raw lineage drift", () => {
  const db = fixture();
  try {
    assert.throws(
