...
test("network-only forward reads selected rows only from the T-5 checkpoint", () => {
  const source = readFileSync("scripts/audit-t5-network-only-forward.ts", "utf8");
  const oddsQuery = source.slice(source.indexOf("function loadLatestCompleteCaptures"), source.indexOf("function loadResults"));
