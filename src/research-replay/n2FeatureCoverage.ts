...
function raceYear(canonicalRaceKeyValue: string): string {
  try {
    return parseCanonicalRaceKey(canonicalRaceKeyValue).raceDateJst.slice(0, 4);
  } catch {
    throw new Error(`N2_COVERAGE_INVALID_RACE_KEY:${canonicalRaceKeyValue}`);
  }
}
