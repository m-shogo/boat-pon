...
const source = readFileSync("scripts/analyze-condb-switch-historical-closing-odds.ts", "utf8");

test("condB switch analysis runs only after payout preflight and DB launch revalidation", () => {
  const payoutAudit = source.indexOf("const audit = runAudit");
  const handoffIdentity = source.indexOf("CONDB_SWITCH_HISTORICAL_DB_HANDOFF_IDENTITY_INVALID");
  const launchIdentity = source.indexOf("CONDB_SWITCH_HISTORICAL_DB_CHILD_LAUNCH_IDENTITY_INVALID");
  ...
  assert.doesNotMatch(source, /await import\("\.\/analyze-condb-switch-historical-closing-odds-internal"\)/);
});

test("condB switch outputs are identity checked, sanitized, and atomically published", () => {
  const jsonIdentity = source.indexOf("CONDB_SWITCH_HISTORICAL_JSON_OUTPUT_IDENTITY_INVALID");
  const mdIdentity = source.indexOf("CONDB_SWITCH_HISTORICAL_MD_OUTPUT_IDENTITY_INVALID");
  ...
  const tempIdentity = source.indexOf("CONDB_SWITCH_HISTORICAL_JSON_PUBLISH_TEMP_IDENTITY_INVALID");
  const destinationIdentity = source.indexOf("CONDB_SWITCH_HISTORICAL_JSON_PUBLISH_DESTINATION_IDENTITY_INVALID");
