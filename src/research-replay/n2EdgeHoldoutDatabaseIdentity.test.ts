...
function assertPrimaryIdentityBlocked(root: string, primaryDbPath: string): void {
  ...
  const result = readN2EdgeHoldoutSource({ primaryDbPath, sidecarDbPath: sidecar });
  assert.equal(result.status, "BLOCKED");
  assert.deepEqual(result.blockers, ["PRIMARY_HOLDOUT_DB_IDENTITY_INVALID"]);
...
test("edge holdout source rejects a primary leaf symlink before any primary database read", () => {
  withRoot((root) => {
...
test("edge holdout source rejects a primary ancestor symlink before any primary database read", () => {
  withRoot((root) => {
...
test("edge holdout source rejects a hard-linked primary before any primary database read", () => {
  withRoot((root) => {
    const real = join(root, "primary-real.sqlite");
