...
test("ROI skip-policy normal entrypoint checks payout completeness before isolated internal simulation", () => {
  ...
  const launchIdentity = entrypointSource.indexOf("ROI_SKIP_POLICY_DB_CHILD_LAUNCH_IDENTITY_INVALID");
  ...
  assert.ok(launchIdentity > isolatedIdentity);
  ...
  assert.match(entrypointSource, /ROI_SKIP_POLICY_DB_CHILD_LAUNCH_IDENTITY_INVALID/);
  assert.match(entrypointSource, /BOAT_PON_DB_PATH: launchDbPath/);
  assert.doesNotMatch(entrypointSource, /BOAT_PON_DB_PATH: isolatedDbPath/);
