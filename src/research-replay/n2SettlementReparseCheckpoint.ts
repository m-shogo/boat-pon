...
export function buildN2SettlementReparseCheckpointIdentity(input: {
  ...
  const selectedFiles = resolvedSelections.map((path) => ({ name: basename(path), sha256: sha256File(path) }));
  const uniqueBasenames = new Set(selectedFiles.map((entry) => entry.name));
  if (uniqueBasenames.size !== selectedFiles.length) throw new Error("REPARSE_CHECKPOINT_ARCHIVE_BASENAME_DUPLICATE");
  return {
    ...
    selectedFilesDigest: canonicalHash(selectedFiles),
    selectedFileBasenames: selectedFiles.map((entry) => entry.name),
  };
}

export function assertN2SettlementReparseCheckpointIdentity(actual: unknown, expected: N2SettlementReparseCheckpointIdentity): asserts actual is N2SettlementReparseCheckpointIdentity {
  if (typeof actual !== "object" || actual === null || Array.isArray(actual)) throw new Error("REPARSE_CHECKPOINT_IDENTITY_MISSING");
  if (canonicalHash(actual) !== canonicalHash(expected)) throw new Error("REPARSE_CHECKPOINT_IDENTITY_MISMATCH");
}

export function buildN2SettlementReparseCheckpointStateDigest(checkpointIdentity: N2SettlementReparseCheckpointIdentity, state: unknown): string {
  return canonicalHash({ digestVersion: N2_SETTLEMENT_REPARSE_CHECKPOINT_STATE_DIGEST_VERSION, checkpointIdentity, state });
}

function requireNonNegativeSafeInteger(value: unknown, label: string): number {
  if (!Number.isSafeInteger(value) || (value as number) < 0) throw new Error(`REPARSE_CHECKPOINT_COUNT_INVALID:${label}`);
  return value as number;
}

function assertDeltaEntries(value: unknown, label: string): number {
  if (!Array.isArray(value)) throw new Error(`REPARSE_CHECKPOINT_REPORT_TABLE_INVALID:${label}`);
  ...
  for (const entry of value) {
    ...
    if (seen.has(entry[0])) throw new Error(`REPARSE_CHECKPOINT_REPORT_KEY_DUPLICATE:${label}:${entry[0]}`);
    seen.add(entry[0]);
    if (typeof entry[1] !== "object" || entry[1] === null || Array.isArray(entry[1])) throw new Error(`REPARSE_CHECKPOINT_REPORT_ENTRY_INVALID:${label}:${entry[0]}`);
    const delta = entry[1] as Record<string, unknown>;
    const keys = Object.keys(delta).sort();
    if (keys.join(",") !== "false_refund,result_kind,special_addition") throw new Error(`REPARSE_CHECKPOINT_REPORT_ENTRY_INVALID:${label}:${entry[0]}`);
  ...
  if (!Number.isSafeInteger(total)) throw new Error(`REPARSE_CHECKPOINT_REPORT_TOTAL_INVALID:${label}`);
  return total;
}

function assertN2SettlementReparseStateAggregates(state: Record<string, unknown>): void {
  const counts = state.counts;
  if (typeof counts !== "object" || counts === null || Array.isArray(counts)) throw new Error("REPARSE_CHECKPOINT_COUNTS_INVALID");
  const countRecord = counts as Record<string, unknown>;
  for (const [key, value] of Object.entries(countRecord)) requireNonNegativeSafeInteger(value, key);
  ...
  if (filesScanned !== filesIngested + filesNotIngested + duplicateSource + parseErrors) throw new Error("REPARSE_CHECKPOINT_FILE_COUNTS_INCONSISTENT");

  const processedFiles = state.processedFiles;
  const processedRawDocs = state.processedRawDocs;
  if (!Array.isArray(processedFiles) || !Array.isArray(processedRawDocs)) throw new Error("REPARSE_CHECKPOINT_PROCESSED_LINEAGE_INVALID");
  if (filesIngested !== processedFiles.length || filesIngested !== processedRawDocs.length) throw new Error("REPARSE_CHECKPOINT_PROCESSED_LINEAGE_COUNT_MISMATCH");

  const seenRawDocs = new Set<string>();
  for (const rawDocumentId of processedRawDocs) {
    if (typeof rawDocumentId !== "string" || rawDocumentId.trim() !== rawDocumentId || rawDocumentId.length === 0) throw new Error(`REPARSE_CHECKPOINT_PROCESSED_RAW_INVALID:${String(rawDocumentId)}`);
    if (seenRawDocs.has(rawDocumentId)) throw new Error(`REPARSE_CHECKPOINT_PROCESSED_RAW_DUPLICATE:${rawDocumentId}`);
  ...
  if (byYearTotal !== appendedCandidates || byBetTypeTotal !== appendedCandidates) throw new Error("REPARSE_CHECKPOINT_REPORT_TOTAL_MISMATCH");
  if (!Array.isArray(state.corrections) || state.corrections.length !== Math.min(appendedCandidates, 400)) throw new Error("REPARSE_CHECKPOINT_CORRECTION_COUNT_MISMATCH");
}

function assertN2SettlementReparseProcessedFiles(checkpointIdentity: N2SettlementReparseCheckpointIdentity, state: unknown): void {
  if (typeof state !== "object" || state === null || Array.isArray(state)) throw new Error("REPARSE_CHECKPOINT_STATE_INVALID");
  const stateRecord = state as Record<string, unknown>;
  const processedFiles = stateRecord.processedFiles;
  if (!Array.isArray(processedFiles)) throw new Error("REPARSE_CHECKPOINT_PROCESSED_FILES_INVALID");
  const allowed = new Set(checkpointIdentity.selectedFileBasenames);
  const seen = new Set<string>();
  for (const file of processedFiles) {
    if (typeof file !== "string" || basename(file) !== file || !allowed.has(file)) throw new Error(`REPARSE_CHECKPOINT_PROCESSED_FILE_OUT_OF_SELECTION:${String(file)}`);
    if (seen.has(file)) throw new Error(`REPARSE_CHECKPOINT_PROCESSED_FILE_DUPLICATE:${file}`);
    seen.add(file);
  }
  assertN2SettlementReparseStateAggregates(stateRecord);
}

export function assertN2SettlementReparseCheckpointStateDigest(actualDigest: unknown, checkpointIdentity: N2SettlementReparseCheckpointIdentity, state: unknown): asserts actualDigest is string {
  if (typeof actualDigest !== "string" || !/^[0-9a-f]{64}$/.test(actualDigest)) throw new Error("REPARSE_CHECKPOINT_STATE_DIGEST_MISSING");
  const expectedDigest = buildN2SettlementReparseCheckpointStateDigest(checkpointIdentity, state);
  if (actualDigest !== expectedDigest) throw new Error("REPARSE_CHECKPOINT_STATE_DIGEST_MISMATCH");
