...
test("selection profile rejects payout selection semantic drift", () => {
  const db = fixture();
  try {
    db.prepare("UPDATE race_payout_lines_v2 SET selection_raw='2-1-3',selection_normalized='2-1-3' WHERE candidate_id='candidate'").run();
    assert.throws(
      () => readN2SelectionProfileSource(db, "2026-05"),
      /N2_SELECTION_PROFILE_PAYOUT_SELECTION_SEMANTICS_INVALID:candidate/u,
    );
  } finally {
    db.close();
  }
...
test("selection profile rejects non-canonical refund selections", () => {
  const db = fixture();
  try {
    db.prepare("INSERT INTO race_refund_lines_v2 VALUES ('refund','candidate',1,'trifecta','9-9-9','9-9-9','9-9-9','selection',100)").run();
    assert.throws(
      () => readN2SelectionProfileSource(db, "2026-05"),
...
test("selection profile rejects refund selection semantic drift", () => {
  const db = fixture();
  try {
    db.prepare("INSERT INTO race_refund_lines_v2 VALUES ('refund','candidate',1,'trifecta','2-1-3','2-1-3','1-2-3','selection',100)").run();
    assert.throws(
      () => readN2SelectionProfileSource(db, "2026-05"),
      /N2_SELECTION_PROFILE_REFUND_SELECTION_SEMANTICS_INVALID:candidate/u,
    );
  } finally {
    db.close();
  }
...
test("selection profile rejects unknown refund scopes", () => {
  const db = fixture();
  try {
    db.prepare("INSERT INTO race_refund_lines_v2 VALUES ('refund','candidate',1,'trifecta','1-2-3','1-2-3','1-2-3','unknown',100)").run();
    assert.throws(
      () => readN2SelectionProfileSource(db, "2026-05"),
...
test("selection profile fails closed when sqlite cannot represent an unsafe refund as a JavaScript number", () => {
  const db = fixture();
  try {
    db.prepare("INSERT INTO race_refund_lines_v2 VALUES ('refund','candidate',1,'trifecta','1-2-3','1-2-3','1-2-3','selection',?)")
      .run(Number.MAX_SAFE_INTEGER + 1);
    assert.throws(
      () => readN2SelectionProfileSource(db, "2026-05"),
      /too large to be represented as a JavaScript number/u,
...
test("selection profile rejects fractional refund amounts", () => {
  const db = fixture();
  try {
    db.prepare("INSERT INTO race_refund_lines_v2 VALUES ('refund','candidate',1,'trifecta','1-2-3','1-2-3','1-2-3','selection',1.5)").run();
    assert.throws(
      () => readN2SelectionProfileSource(db, "2026-05"),
