...
function fixture(popularity: number): DatabaseSync {
  ...
  const semanticHash = canonicalHash({
    ...
    resultKind: "normal",
    payouts: [["1-2-3", 1000, popularity, "payout"]],
    refunds: [],
  });
  ...
  db.prepare("INSERT INTO race_payout_lines_v2 VALUES (?,?,?,?,?,?,?,?,?)")
    .run("candidate", 1, "trifecta", "1-2-3", "1-2-3", "1-2-3", 1000, popularity, "payout");
  return db;
}

test("semantic hash validation accepts producer-valid line semantics", () => {
  const db = fixture(1);
  try {
...
test("semantic hash validation rejects matching hashes built from producer-impossible line semantics", () => {
  const db = fixture(0);
  try {
