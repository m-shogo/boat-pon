...
function openImmutable(path: string): DatabaseSync {
  if (!existsSync(path)) throw new Error("SIDECAR_DB_NOT_FOUND");
  const lexicalPath = resolve(path);
  const lstat = lstatSync(lexicalPath);
  if (lstat.isSymbolicLink() || !lstat.isFile()) throw new Error("SIDECAR_DB_IDENTITY_INVALID");
  const stat = statSync(lexicalPath);
  if (!stat.isFile() || stat.nlink !== 1 || realpathSync(lexicalPath) !== lexicalPath) {
    throw new Error("SIDECAR_DB_IDENTITY_INVALID");
  }
