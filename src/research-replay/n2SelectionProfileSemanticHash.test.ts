...
const RACE_KEY = "2026-05-01:01:R1";

test("selection profile rejects a current settlement with a tampered semantic hash", () => {
  ...
  try {
    ...
    db.prepare(`
      INSERT INTO domain_observations VALUES (
        'obs', ?, 'settlement_result', 'settlement_result', 'raw', 'parse', NULL, NULL, NULL
      )
    `).run(RACE_KEY);
    db.prepare(`
      INSERT INTO settlement_candidates_v2 VALUES (
        'candidate', ?, 'trifecta', 'settled', 'normal', 'initial', 'resolved',
        'official_result', 'n1-settlement.0.1', 'obs', 'parse', 'raw', ?, NULL, NULL,
        '2026-05-01T04:00:00.000Z', '2026-05-01T04:00:00.000Z'
      )
    `).run(RACE_KEY, "0".repeat(64));
    db.prepare(`
      INSERT INTO race_payout_lines_v2 VALUES (
        'payout', 'candidate', 1, 'trifecta', '1-2-3', '1-2-3', '1-2-3', 1000, 1, 'payout'
      )
    `).run();

    assert.throws(
      () => readN2SelectionProfileSource(db, "2026-05"),
      /N2_SELECTION_PROFILE_SETTLEMENT_SEMANTIC_HASH_INVALID:candidate/u,
    );
  } finally {
    db.close();
  }
