...
function withTempDb(run: (path: string) => void): void {
  const dir = mkdtempSync(join(tmpdir(), "boat-pon-real-plan-venue-boundary-"));
...
test("real plan preflight rejects unknown venues before the three-date bound", () => {
  withTempDb((path) => {
    ...
    try {
      ...
      insert.run("20261001-99-01", "2026-10-01", "99", 1, "12:00");
    ...
    assert.ok(report.blockers.includes("OFFICIAL_PROGRAM_VENUE_INVALID:2026-10-01:99"));
