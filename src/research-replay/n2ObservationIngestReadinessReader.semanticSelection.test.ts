...
function createPrimary(path: string): void {
  ...
  try {
    ...
    for (const selection of trifectaSelections().slice(0, 60)) {
      insertOdds.run("20260805-01-01", "trifecta", selection, 10, "2026-08-05T02:30:00.000Z", "T-30");
      insertOdds.run("20260805-01-01", "trifecta", ` ${selection}`, 10, "2026-08-05T02:30:00.000Z", "T-30");
    }
  } finally {
    db.close();
...
test("raw formatting variants cannot inflate semantic trifecta coverage to 120 selections", () => {
  const root = mkdtempSync(join(tmpdir(), "n2-readiness-semantic-selection-"));
