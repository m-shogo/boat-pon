...
test("ROI mechanism skip-filter raw analyzer cannot bypass settlement preflight or provenance redaction", () => {
  assert.match(wrapper, /audit-roi-mechanism-skip-filter-payout-completeness\.ts/);
  assert.match(wrapper, /await import\("\.\/analyze-roi-mechanism-skip-filters-internal"\)/);
  assert.doesNotMatch(wrapper, /analyze-roi-mechanism-skip-filters-raw/);
  assert.match(wrapper, /redactDbProvenance\(handoffDbPath\)/);
