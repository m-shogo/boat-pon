...
test("skip6r switch analysis runs only after payout preflight and DB launch revalidation", () => {
  const payoutAudit = source.indexOf("const audit = runAudit");
  const handoffIdentity = source.indexOf("SKIP6R_SWITCH_HISTORICAL_DB_HANDOFF_IDENTITY_INVALID");
  const launchIdentity = source.indexOf("SKIP6R_SWITCH_HISTORICAL_DB_CHILD_LAUNCH_IDENTITY_INVALID");
  ...
  assert.doesNotMatch(source, /await import\("\.\/analyze-skip6r-switch-historical-closing-odds-internal"\)/);
});

test("skip6r switch outputs are identity checked, sanitized, and atomically published", () => {
  const jsonIdentity = source.indexOf("SKIP6R_SWITCH_HISTORICAL_JSON_OUTPUT_IDENTITY_INVALID");
  const mdIdentity = source.indexOf("SKIP6R_SWITCH_HISTORICAL_MD_OUTPUT_IDENTITY_INVALID");
  ...
  const tempIdentity = source.indexOf("SKIP6R_SWITCH_HISTORICAL_JSON_PUBLISH_TEMP_IDENTITY_INVALID");
  const destinationIdentity = source.indexOf("SKIP6R_SWITCH_HISTORICAL_JSON_PUBLISH_DESTINATION_IDENTITY_INVALID");
