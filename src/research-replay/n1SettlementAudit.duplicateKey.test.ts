...
test("N1 legacy reconciliation fails closed on duplicate settlement keys", () => {
  const root = mkdtempSync(join(tmpdir(), "boat-pon-n1-duplicate-"));
  ...
  try {
    ...
    insert.run(sample.raceId, sample.betType, sample.combination, Number(sample.payoutYen) + 100);
  } finally {
    db.close();
  }
  ...
  try {
    assert.throws(
      () => reconcileSanitizedKFixture(dbPath, fixturePath),
      /N1_LEGACY_SETTLEMENT_DUPLICATE_KEY/,
    );
  } finally {
