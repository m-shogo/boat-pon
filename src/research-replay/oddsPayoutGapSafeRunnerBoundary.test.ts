...
test("odds-payout-gap preflight fails closed on decision cohort drift before ROI analysis", () => {
  ...
  assert.ok(cohortGuard >= 0 && cohortGuard < completenessGuard, "decision cohort drift must fail closed before payout completeness is accepted");
});
