...
function runCase(input: {
  ...
  try {
    ...
    assert.deepEqual(terminal, [{ event_kind: "capture_failed", failure_reason: "partial_body" }]);
    assert.equal((db.prepare("SELECT COUNT(*) n FROM capture_raw_links").get() as { n: number }).n, 0);
