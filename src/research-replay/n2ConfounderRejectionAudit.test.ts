...
function result(id:string,verdict:N2EdgeHistoricalConfirmationResult["verdict"]):N2EdgeHistoricalConfirmationResult{
 const split={split:"validation" as const,uniqueRaceCount:220,meanResidual:.02,standardError:.002,zScore:10,rawPValue:1e-10,holmAdjustedPValue:1e-10,supportSufficient:true,effectSufficient:true,directionMatchesDiscovery:true,statisticallyConfirmed:true};
 return{hypothesisId:id,featureKey:"firstCourse",bucket:"1",discoveryDirection:"underpredicted",validation:split,test:{...split,split:"test"},verdict};
}
