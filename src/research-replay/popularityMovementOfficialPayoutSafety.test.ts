...
test("popularity movement report rejects unsupported bet types across the full report population before popularity/ROI aggregation", () => {
  assert.match(source, /function assertSupportedBetTypeMapping/u);
  assert.match(source, /POPULARITY_MOVEMENT_REPORT_BET_TYPE_MAPPING_FAILED/u);
