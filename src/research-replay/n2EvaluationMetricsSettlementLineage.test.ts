...
function insertCandidate(db: DatabaseSync, input: {
  ...
  db.prepare(`INSERT INTO race_payout_lines_v2
    ...
    VALUES (?,?,1,'trifecta','1-2-3','1-2-3','1-2-3',1230,NULL,'payout')`)
...
test("reader rejects settlement evidence from a failed parse run", () => {
  withDb((path, db) => {
    insertObservation(db, {
      observationId: "obs-a",
      raceKey: "2026-08-07:05:R1",
    ...
    db.prepare("UPDATE parse_runs SET status='error' WHERE parse_run_id='parse-a'").run();
    insertCandidate(db, {
      candidateId: "candidate-a",
      raceKey: "2026-08-07:05:R1",
      observationId: "obs-a",
