...
N0後の実装順序と研究基盤は[`research-platform-master-plan.md`](research-platform-master-plan.md)を最上位正本とする。Stage F0は五層lineageとPIT/leakage guard、Stage F0-Rは独立sidecar rollout、failure isolation、backup/restore、operational retentionを実装して`COMPLETE`。詳細は[`research-replay-foundation.md`](research-replay-foundation.md)と[`research-replay-rollout.md`](research-replay-rollout.md)。`data/boat.sqlite`変更や新規収集は行っていない。次はN1 schema/migration再レビューで、別承認まで実装しない。
