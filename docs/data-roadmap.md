...
N0後の実装順序と研究基盤は[`research-platform-master-plan.md`](research-platform-master-plan.md)を最上位正本とする。Stage F0は五層lineageとPIT/leakage guard、Stage F0-Rは独立sidecar rollout、failure isolation、backup/restore、operational retentionを実装して`COMPLETE`。承認gateは`f0r-approval-v2`へhardening済み。詳細は[`research-replay-foundation.md`](research-replay-foundation.md)と[`research-replay-rollout.md`](research-replay-rollout.md)。N1 schema/migration実装前レビューは[`n1-all-bet-type-payout-review.md`](n1-all-bet-type-payout-review.md)で`CONDITIONAL`完了。N1-A offline foundationは`COMPLETE`、N1-B Permanent Settlement Schema Rolloutは`CONDITIONAL`で`n1-settlement.0.1`を永続sidecarへzero-dataで適用済み（[`n1-settlement-permanent-rollout.md`](n1-settlement-permanent-rollout.md)）。full backfill projected ≈10.5GBが1GiB quota超過のため、evidence pin Option B・`n1-settlement.0.2` checkpoint schema・backfill executorをN1-C準備として実装。N1-C Persistent Backfillは**実行済み（Backfill=COMPLETE / Final verification=COMPLETE / acceptance=CONDITIONAL）**: quotaを30GBへ引き上げ、0.2適用、Option Bで全券種を段階backfill（8,168/8,168完了・failed 0）、candidates 8,154,709 / evidence pins 0 / integrity ok / fk 0 / observation-level dup 0 / legacy mismatch 0、最終DB≈9.0GB。closure（2026-07-29）で authoritative verify と +5,153 完全 reconciliation（unexplainedDelta=0）を解消。4 source file（2008-2009）の intra-file 重複（race-level 重複 candidate 4,196、source-data defect・値誤りなし）を **append-only source_duplicate resolution（`n1-settlement.0.3`）で canonical 解決**（raw immutable・active canonical 重複 0/0・冪等）。恒久 invariant（active canonical race-level uniqueness=0）組込。live-archive 日次追加も incremental（8,170/8,170）。正本は[`../reports/n1c-backfill/n1c-final-report.md`](../reports/n1c-backfill/n1c-final-report.md)、finding/resolution は[`../reports/n1c-backfill/data-quality-finding-duplicate-source-archives.md`](../reports/n1c-backfill/data-quality-finding-duplicate-source-archives.md)、runbookは[`n1-settlement-backfill-runbook.md`](n1-settlement-backfill-runbook.md)。`data/boat.sqlite`はN1 write 0（read-only fingerprintのみ）。低water 24GB化・GC・future collector・N2は別承認待ち。
