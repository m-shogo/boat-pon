...
### レース番号 2・8 が系統的に悪い（B1+25-30 内）

B1+25-30帯をレース番号別に集計（2024+2025合算）:
...
- race 2+8 除外: n=813, hit_pct=3.94%, ROI=1.419
- 理論的根拠: 不明（番組編成パターンか、女子戦・企画戦の集中？）
- 現状 BetCandidate に raceNo フィールド実装済み → `excludedRaceNos` として実装可能
- **2023年外部検証が必要。理論的説明なしに採用しない**

### B1+B1+B1 配艇（2・3号艇も B1）は一段悪い

| config | n | 2024 hit_pct | 2024 ROI | 2025 hit_pct | 2025 ROI |
|--------|---|-------------|----------|-------------|----------|
| B1+B1+B1 | 265 | 2.91% | 0.974 | 1.08% | 0.326 |
| other | 772 | 3.43% | 1.255 | 3.99% | 1.448 |

- B1+B1+B1: 全3艇B1クラス → クラス差なし → アップセット率が高い
- 「クラスの差がない組み合わせは予測困難」という理論的動機あり
