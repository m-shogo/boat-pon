...
**paper live観察モード（実購入なし）**

- 外部検証 ROI=0.939 < 1.0 → edge未確認 → 実購入しない
- 判定・オッズ取得・decision_history・結果記録はすべて継続
- BUY通知は `[paper] BUY候補` として発火するが、実購入の根拠にしない
- パラメータ最適化の余地は外部検証で尽きた。コード変更は不要

## 再検討条件（この3条件を同時に満たすまで購入しない）

1. live BUY n >= 300
2. live ROI > 1.05
3. roiExMax（最大払戻除外ROI）> 1.0 が望ましい

n < 300 の段階では ROI がどう出ても購入判断しない（3連単の分散が大きすぎる）。
