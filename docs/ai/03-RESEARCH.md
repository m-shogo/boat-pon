...
| Daily Research Report | 🟡 | `src/domain/dailyResearchReport.ts` + `pnpm daily:research-report`（Phase 5最小実装: ROI Explorerとdetect:driftの結果を1つの研究レポートに要約、`--json`/`--presentation-json`、read-only CLI）。Phase 5.1で`--rules-file`による複数ルール横断レポート（`DailyResearchReportAggregate`）を追加。Phase 5.2で`ResearchRule.evaluationConditions`（equalsのみ、allowlist方式、`src/domain/researchRuleConditions.ts`）に基づくrule-specific ROI/Drift評価に対応（条件なし/無効な条件はshared-fallback/invalid-condition-fallbackへ安全にフォールバック）。`data/research-rules.json`は**読み取り専用**（archivedは既定で除外、実候補はAIが勝手に追加しない）。**買い推奨・Production昇格の判断ではない**。新仮説/Opportunity/`docs/rule-candidates.md`との統合、任意条件（範囲・OR・正規表現）は未着手 |
