...
| D. feature PIT | **READY(enforcement+lineage+typed parser/byte-exact offline ingest+immutable coverage reader) / BLOCKED(real observations)** | namespaced class laundering拒否。strict `official_program` parser、typed schema/hash/identity/observed_atとprimary semantic値の照合、primary raw bytes→temp F0 sidecar→coverage 42/42 E2Eを実装。正規化envelopeをraw原本へ置換せず、不正rawはerror parse runのみ。関連tests 9/9 PASS。実collector sidecar writeと実profileはPENDING |
