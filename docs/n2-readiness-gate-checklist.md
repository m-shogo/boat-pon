...
| D. feature PIT | **READY(enforcement+lineage+typed contract+immutable coverage reader) / BLOCKED(real observations)** | namespaced class laundering拒否。strict `official_program` pre-race payload、primary+F0のimmutable/read-only証拠鎖reader、feature×年代coverage集計を実装。canonical join key不一致も修正し隔離SQLite E2EはPASS。実collector observationと実profileはPENDING |
