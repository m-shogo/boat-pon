...
| D. feature PIT | **READY(enforcement+lineage+immutable coverage reader) / BLOCKED(real observations)** | namespaced class laundering拒否。primary+F0をimmutable/read-onlyで開く証拠鎖readerとfeature×年代coverage集計を実装。隔離SQLite E2EはPASS。現typed registryにofficial_program observationなし、実profileはPENDING |
