...
| D. feature PIT | **READY(enforcement+lineage+typed parser/verification+immutable coverage reader) / BLOCKED(real observations)** | namespaced class laundering拒否。strict `official_program` parser/envelope、typed schema/hash/identity/observed_atとprimary semantic値の照合、primary+F0 immutable reader、feature×年代coverageを実装。欠損rateの0化とcanonical join key不一致も修正し対象tests 17/17 PASS。実collector sidecar writeと実profileはPENDING |
