...
最終更新: 2026-05-26 (セッション13 v4検証・不採用・v3に戻す)
...
- 2026年 v3-alpha15 live BUY: **n=0**（まだ蓄積なし）
- 2026-05-20 の旧モデルBUY 43件、2026-05-21 の sample 3件は採用判断から除外する
- 2026-05-26 は v3 判定131件が保存されたが、129件はオッズ未取得SKIP。これは採用/撤退判断の n には入れない
- ライブ蓄積ペースは未確定。2025年 `history-model` の月間BUY数は事後評価であり、Mac稼働・取得タイミング・オッズ取得失敗を含む実ライブペースとは別物
- まず1週間は `monitor:live` の `liveDecisionOdds` でオッズ取得率を監視し、未取得が多い場合はモデルではなく取得運用を直す
