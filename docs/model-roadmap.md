...
不採用結果も研究台帳として保持する。F後180日、雪辱戦、相手にF後が複数、当地覚醒は、今回の定義ではBUY根拠にしない。これらから得た重要な知見は「物語として自然」と「市場控除後に再現するedge」は別物だということ。名前や閾値だけを変えて同じ探索を繰り返さず、再検証には新しいpoint-in-time情報、事前固定した定義、独立期間を要求する。詳細は`reports/unconventional-feature-screen.md`の「不採用から得た知見」に残す。
