...
- N1-C closure（2026-07-29）で2つのverification debtを解消: (A) 現8,168 stateへ authoritative full verify を完走（integrity ok / fk 0 / observation-level dup 0 / coverage 8,168/8,168 / pins 0）、(B) +5,153 line delta を完全 reconciliation（unexplainedDelta=0 / simMatchesDb=true / parser determinism 0）。ただし reconciliation が **source archive の intra-file 重複**を surface: 4 file（2008-07-06/07-13, 2009-04-06/07-08）が日次データを物理重複格納 → race-level 重複 candidate **4,196** / dup observation 624 / dup line 11,658（store約0.05%）。source-data defect・値誤りなし・完全説明済みだが race単位重複が未解決のため **COMPLETE へ昇格せず CONDITIONAL**。正本 `reports/n1c-backfill/data-quality-finding-duplicate-source-archives.md`。破壊的修正は未実施。
...
- N1-C後のgate: quota 16GB+/low-water 24GB+へ再引き上げ（GC・追加ingest前）、live archive日次incremental backfill運用、operational GC有効化は専用readiness gate（`docs/n1-settlement-gc-safety-contract.md`）+別承認、future collector/N2は別承認。
