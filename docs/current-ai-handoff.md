...
- **N2 readiness（2026-07-30、設計のみ・model training未着手）**: N2着手前の契約・PIT・leakage・評価・split・reproducibilityを設計＋enforcement＋prototypeまで完了し **N2_IMPLEMENTATION_READY=YES**。正本: `docs/n2-data-contracts.md`・`docs/n2-feature-pit-contract.md`・`docs/n2-evaluation-and-split.md`・`docs/n2-readiness-gate-checklist.md`（A–N gate）、enforcement `src/research-replay/n2DatasetContract.ts`(+test)、profile `reports/n2/n2-dataset-profile.json`（eligible 96.03%）、audit `reports/n2/n2-readiness-audit.md`。重要所見: **2001–2003 archive欠落**（source完全性gap、N1-C不良ではない）、eligibility era drift 87%→99.9%、既存ROI研究の再分類（racer_course_stats由来=LEAKY、current_odds gap=CONDITIONAL、calibration方法論=SUPPORTED）。model training/feature build/production/GC/N2実装は別承認。
