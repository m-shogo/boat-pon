...
- **N2 target contract v2（2026-08-01）**: `deriveBetLabel` は `outcome=hit/loss/refund/special_payout/void` を返す。部分返還selectionと特払いをclassification lossから分離し、refund/special_payout/voidでは`hit=null`。実額不明の返還を100円へ推測しない。Node 24の契約testは8 pass / 0 fail。次はraw scanner実行とcanonical reconciliation、その後7券種全selection prototype。
