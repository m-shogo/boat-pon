...
- Phase N1-C（Persistent Sidecar Backfill）は**Backfill execution=COMPLETE / Overall=CONDITIONAL**。`n1-settlement.0.2`（checkpoint table、expand-only、0.1不変）を永続sidecarへ適用し、Option B（`emitEvidencePins=false`）で全券種settlementを段階backfill（1→10→100→1000→8,167）した。実行時manifest **8,167/8,167完了・failed 0**、現archive **8,168/8,168**（post-run日次file k260726を増分適用）。candidates **8,154,709**、payout ~11.07M、refund ~446k、**evidence pins 0**、integrity ok / fk 0 / semantic duplicate 0 / legacy payout mismatch 0 / deterministic rerun 追加0。最終DB **≈9.0GB**（sample予測5.38GBから+67%上振れ、要quota再評価）。正本は`reports/n1c-backfill/n1c-final-report.md`、運用は`docs/n1-settlement-backfill-runbook.md`。
