...
- **N2 legacy source adapter（2026-08-01）**: primary DBを再監査すると`official_programs`は`imported_at`のみでsource availability/raw lineageなし、odds時系列もcaptured_atのみでraw lineageなし、既存schemaはbet_type未固定だった。`n2FeatureSourceAdapter.ts`を追加し、race日/imported_atのavailability代用、bet_type推測、lineage欠損をすべて拒否。captured_atはraw lineageがある場合のみ保守的にavailable_at同値として扱う。adapter tests 6/6、namespaced launderingを含むbuilder regression 8/8、strict typecheck PASS。次はF0 evidenceとのread-only lineage joinとcoverage/provenance report。
