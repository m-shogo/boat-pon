...
- **N2 target contract v2 / selection foundation（2026-08-01）**: `deriveBetLabel` は `outcome=hit/loss/refund/special_payout/void` を返し、refund/special_payout/voidでは`hit=null`。全7券種のcanonical selectionを6/6/30/15/120/20/15（計212）で決定順列挙し、`deriveSelectionLevelLabels`で全selectionをlabel関数へ通す。targeted strict typecheck PASS、Node 24 contract tests 10 pass / 0 fail。実DBのselection-level class balance/的中率/payout分布、独立DB再読込rebuild、raw scanner/canonical reconciliationは未完了。
